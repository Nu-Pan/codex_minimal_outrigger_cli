...
- 要求
    - [推奨] 必ず守らなければならない要件を満たしている範囲内（解空間内）で、realization files 全体で見た時の総文字数が最小となること（文字数最小解）を目指す
    - [推奨] 複数箇所で出現するよく似た処理は関数化し、それを各所で使い回す
    - [推奨] 意味的に重複している実装は 1 つに集約する
    - [必須] 現行の仕様と関係のない、過去の仕様に基づく実装は削除する
- 判定
...
- 要求
    - [推奨] realization files は、現行仕様を満たすために必要な情報だけを持つ
    - [推奨] realization code は、責務・入力・出力・副作用が読み取りやすい単位に分割する
    - [禁止] 読み取りやすさを損なう過度な圧縮をしてはいけない
    - [禁止] 将来使うかもしれないだけの抽象化を追加してはいけない
    - [推奨] 実装だけから読み取りにくい意図・制約・根拠は、コメントまたは docstring に書く
    - [禁止] シグネチャ・型名・実装から明らかな情報だけを、コメントや docstring で冗長に繰り返してはいけない
    - [必須] 現行仕様・現行実装から不要になった realization files、関数、クラス、分岐、設定、テスト、コメントは削除する
- 判定
...
- 要求
    - [必須] realization code を追加する前に、同じ責務または近い責務を持つ既存の realization code を確認する
    - [禁止] 既存実装の修正・移動・置換で現行仕様を満たせる場合は、新しい実装を追加してはいけない
    - [必須] 新しい実装を追加した場合は、置き換えられた古い実装、不要になった分岐、重複した定数、古いテストも同時に削除または統合する
    - [禁止] 一時的な移行コード・互換コードは、現行仕様から明示的に必要な場合以外は残してはいけない
    - [必須] 移行コード・互換コードを残す場合は、それを削除できる条件を近くのコメントに書く
- 判定
...
- 要求
    - [必須] 新しい関数・クラス・モジュールは、現行仕様上の責務境界を明確にする場合、または実在する重複を削減する場合に作る
    - [禁止] 単に処理が短いという理由だけで、1 箇所からしか呼ばれない小関数を増やしてはいけない
    - [必須] 共通化は、見た目が似ているだけではなく、入力・出力・失敗時挙動・不変条件が同じ場合に行う
    - [禁止] 呼び出し元ごとの差分を大量のフラグや callback で吸収する汎用関数を作ってはいけない
    - [推奨] 共有範囲が 1 モジュール内に閉じる場合は、そのモジュール内の private helper に留める
    - [必須] 共有モジュールへ移動するのは、複数のサブコマンドまたは複数の上位モジュールから使うことが明確な場合に限る
- 判定
...
- 要求
    - [必須] 新しい CLI 引数、サブコマンド、設定項目、環境変数、出力項目、状態ファイルは、現行仕様を満たすために必要な場合だけ追加する
    - [禁止] 内部実装の都合だけで公開面を増やしてはいけない
    - [推奨] 単一の妥当な挙動を選べる場合は、設定項目で利用者に選ばせない
    - [必須] 新しい永続状態を追加する場合は、その生成条件・更新条件・削除条件を仕様または実装上で明確にする
    - [禁止] 同じ情報をログ・状態ファイル・出力 schema に重複して保存してはいけない
- 判定
...
- 要求
    - [必須] テストは、実装詳細ではなく、現行仕様上意味のある外部挙動または制御ロジックを検証する
    - [推奨] 同じ観点のテストケースは、可能な範囲で parametrized test や小さな fixture に集約する
    - [必須] 似たテストを追加する前に、既存テストへケース追加できないか確認する
    - [必須] 古い仕様に対応するテスト、削除済み実装に対応するテスト、同じ失敗を重複して検出するテストは削除または統合する
    - [推奨] 大きな fixture ファイルは、現行仕様の検証に必要な最小内容にする
    - [禁止] Codex CLI や LLM の出力品質そのものを検証するためのテストを追加してはいけない
- 判定
...
- 要求
    - [必須] 新しい外部依存は、標準ライブラリまたは既存依存で現行仕様を十分に満たせない場合だけ追加する
    - [推奨] 外部依存を追加する場合は、それにより自前実装・テスト・保守負担がどれだけ減るかを重視する
    - [禁止] 小さな処理のためだけに大きな依存を追加してはいけない
    - [禁止] 生成可能なキャッシュ、実行ログ、一時ファイル、ビルド成果物を realization files として持ってはいけない
    - [必須] 補助スクリプトを追加する場合は、既存の CLI・テスト・開発手順で代替できないか確認する
- 判定
...
- 要求
    - [必須] realization files の変更完了前に、追加したもののうち削除・統合・短縮できるものがないか確認する
    - [禁止] 未使用の関数、クラス、import、定数、fixture、補助ファイルを残してはいけない
    - [禁止] 同じ責務を持つ名前違いの実装を残してはいけない
    - [禁止] 現行仕様の説明に不要な TODO、NOTE、コメントを残してはいけない
    - [推奨] 変更後の realization files は、現行仕様を満たすための最小構成にする
- 判定
