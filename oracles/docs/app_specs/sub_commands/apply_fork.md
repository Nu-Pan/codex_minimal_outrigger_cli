...
- この apply ループは以下の状態を目標とする
    - `<work-root>` の実装が `<work-root>/oracles` の正本仕様断片と一致している
...
3. apply ループ
    ...
    5. 修正作業ループ（改善後の要修正点リストに対する for-each）
        ...
        2. `<work-root>/oracles` などの編集禁止ディレクトリに未コミット差分が有る場合はエラー終了
...
### 調査対象 oracles files の snapshot 原則
...
- `<cmoc-apply-branch>` の HEAD を調査対象とする
    ...
    - ところで、apply 系サブコマンドでは oracles files の編集は行われないから、「`<cmoc-apply-fork-commit>` を調査対象とする」と「`<cmoc-apply-branch>` の HEAD を調査対象とする」は意味論的には同値である
...
- `cmoc apply fork` の対象なるファイルの候補は「oracles files 全て」「実装ファイル全て」である
...
- このファイル起点の依頼は、事前に列挙したファイルリスト上のファイル全てに対して個別に行う
    - i.e. 調査対象となる oracles files が N 件と実装ファイルが M 件存在するのであれば `codex exec` を N + M 回呼び出すということ
- このファイル起点の依頼は並列に実行する
...
- oracles files と実装との明確な不整合
    - 「oracles files 上で記述されている仕様」と「実装」とが明確に不整合している点を指す
...
- 実装上の明確な問題点
    ...
    - 当然ながら、修正後の実装は oracles files 上で記述されている仕様を満たしている必要がある
