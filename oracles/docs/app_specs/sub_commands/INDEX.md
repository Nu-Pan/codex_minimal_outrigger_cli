...
- インデクシングそのものの一般仕様は別紙の `<work-root>/oracles/docs/app_specs/indexing.md` に分かれており、このファイルはサブコマンド側の入口として機能します。
...
- 単に `oracles` 配下の他の仕様やルーティング文書をたどりたいだけで、`cmoc indexing` の仕様は不要なとき。
