...
- 現在の `<work-root>/oracles` に致命的な問題が無いかレビューし、レビュー結果を人間にレポートする
...
- `cmoc review oracles` の責務であること
    - oracles files に対する所見をリスト化して人間にレポートとして提示する
    - 人間の認知コストを節約するために、指定された最大回数の範囲内で所見リストを高品質化する
    - oracles files に基づいたレビューを行う
    - oracles files 上の「間違っていることが明白な要素」を人間に知らせる
    - oracles files のスナップショットをレビューする
- `cmoc review oracles` の責務ではないこと
    ...
    - 「oracles files もっと良くするには」の人間への提案は目的ではない
    - 過去 oracles files に何があったか (i.e. 編集・追加・削除) はレビュー対象ではない
...
- 単純な問題 (minor) を対象とする
    ...
    - その他ケアレスミスの疑いが濃厚なもの
- 以下の問題は対象としない
    - oracles files だけからは問題だとは言い切れない
    - 仕様からは実装が一意に定まらない
...
- レビューにあたってエージェントは...
    - `<work-root>/oracles` 配下のファイルを自由に読んで良い
    - `<work-root>/oracles` 以外のファイルは読んではいけない
    - 指定されたファイルだけでなく、関連する oracles files も読みに行く
    - どの oracles files が関連するかは `INDEX.md` を根拠に判断する
...
- oracles files  1 つごとにダーティーフラグを用意する
...
- ダーティーフラグの初期値はスコープモードによって変わる
    ...
    - フルスコープ (`--scope full`) の場合、全ての oracles files のダーティフラグを true とする
...
- 1 回の `codex exec` 呼び出しで 1 つの oracles files をレビューする
...
- `minor` の場合
    - oracles files に、致命的ではない、細かい問題があることを書く
- `ok` の場合
    - oracles files に、問題は何ら見つからなかったことを書く
