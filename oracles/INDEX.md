...
- 802f18ddb49ff4772bd08cb41a96628d666a76816ad3573c3a8d444c6bf6c638

# `schemas`

## Summary

- `<cmoc-root>/oracles/schemas/structured_output/` 配下の Structured Output schema をまとめた目次です。
- 現状は `structured_output/` を案内し、`cmoc review oracles` で使う JSON schema 群へ分岐します。
...
- `cmoc review oracles` 用の JSON schema を追加・修正・レビューしているとき。

## Do not read this when

- `structured_output/` 配下の個別 schema がすでに分かっていて、直接読むとき。
- `cmoc review oracles` の利用手順や評価フローだけを確認したいとき。
