...
- 17b3c5ff23351d3dec209197aae50b25d29267e65627042a13134592aaa9d44d
<!-- cmoc-index-kind: directory -->
...
- pytest の共通設定ファイル `conftest.py` と、cmoc の各機能を検証する回帰テスト群が入っているディレクトリです。
- Codex 呼び出しラッパー、`INDEX.md` メンテナンス、repo 共通処理、レポート保存、タイムスタンプ、ファイル命名、CLI・サブコマンドの挙動を広くカバーします。
- 実装の詳細ではなく、`tests` 配下で何を検証しているかを素早くたどるための入口です。

## Read this when

- `tests` 配下にどんな回帰テストがあるか全体像を把握したいとき。
- 共通 pytest 設定や、`commons`・CLI・`INDEX.md` まわりのテスト入口を探したいとき。
- 新しいテストを追加するときに、このディレクトリの既存テストの役割分担を確認したいとき。

## Do not read this when

- cmoc の実装本体やサブコマンド処理を直接追いたいとき。
- `oracles` 配下の正本仕様や開発ルールだけを確認したいとき。
- 個別のテストケース１件だけを深掘りしたいときで、テスト群全体の案内は不要なとき。

## hash

- a91f29638885667bdcf00e135e4f9b6b5501773238a1b9986848032f90386088
<!-- cmoc-index-kind: directory -->
