...
## Summary
- `src` は realization implementation の上位入口で、cmoc の実行本体を責務別に分けた配下モジュールへ進むためのルーティング領域。
- ここでは実装そのものを説明するのではなく、CLI 入口、共通 runtime、設定、各サブコマンド実装、互換 shim のどこを読むべきかを切り分ける。
- `oracle` 側の正本定義を複製せず、既存公開面からの再公開や委譲で維持される領域が含まれる。

## Read this when
- realization implementation の入口を起点に、どの責務がどの下位モジュールにあるかを判断したいとき。
- CLI の配線、共通 helper、設定、各サブコマンド実装、互換 shim のいずれへ進むべきかを選びたいとき。
- 既存の公開 import 経路や委譲関係を確認し、正本側の実装へ直接進むべきかを見分けたいとき。

## Do not read this when
- 個別機能の挙動や内部処理を確認したいときは、ここではなく該当する下位モジュールを読む。
- 正本仕様そのものや oracle file の内容を確認したいときは、`src` ではなく `oracle` 側を読む。
- すでに読む対象の責務が分かっているときは、上位の案内ではなく直接その実装ファイルへ進む。

## hash
- 37dadb557fc207717f14bda5c2849f85bed4ad24943a05daef5aada0c79b4e99

# `test`

## Summary
- `test` 配下で `acp_builder` 系テストが正本側の path や schema を直接参照するための共通補助。test 側に正本内容を複製せず、`oracle/src/oracle/acp_builder` へつなぐ経路だけをまとめる。

## Read this when
- `acp_builder` の正本定義をテストから辿る共通 helper の挙動を確認・変更したいとき。
- 正本をコピーせず oracle 側を参照する方針が、この helper によって支えられているか確認したいとき。
- `acp_builder` 関連テストの path 解決だけを見たいとき。

## Do not read this when
- 個別の `acp_builder` 仕様や builder 本体を確認したいときは、対応する oracle 側の本文を読む。
- テスト全体の CLI 挙動や他の共通 support を確認したいときは、この helper ではなく該当する別の test support を読む。
- INDEX ルーティング方針そのものを確認したいときは、上位の案内を読む。

## hash
- 36ba62bf4dffce127823a3523cd8bc2ba793d2e4a4b1363a409a94cd03e9e260
