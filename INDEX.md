...
- 上位の仕様断片と実装基盤のどちらから読むべきかを切り分けるための目次です。
- この階層を起点に、各下位ディレクトリの `INDEX.md` や個別仕様へ分岐します。

## Read this when

- cmoc の正本仕様断片がある `oracle` 全体の入口を把握したいとき。
- まず `doc/` と `src/` のどちらに進むべきか整理したいとき。
- 各下位ディレクトリの `INDEX.md` を読む前に、役割分担を確認したいとき。

## Do not read this when

- すでに目的の文書が分かっていて、`doc/` か `src/` の該当文書へ直接進めるとき。
- この階層ではなく、下位ディレクトリの `INDEX.md` や個別仕様だけを確認したいとき。
- `oracle` 全体の入口ではなく、別のルーティング文書や開発規約だけを探しているとき。

## hash

- 081ace3ea7c1ef734040658ee3bd791077e7343bc14948820e3367f644061ab4
