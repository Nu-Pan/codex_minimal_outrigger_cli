...
- cmoc の実装ルートで、CLI 起点・共通処理・サブコマンド実装をまとめるディレクトリです。
- `main.py` は `cmoc` CLI の入口で、`init`、`session`、`apply`、`review` のコマンド登録と例外整形を担当します。
- `commons/` は再利用可能な共通基盤、`sub_commands/` は各サブコマンド本体の入口です。

## Read this when

- cmoc のソース全体で、起点・共通基盤・サブコマンド実装の配置関係を俯瞰したいとき。
- `cmoc` の起動経路やコマンド登録がどこにあるかを確認したいとき。
- 共通処理を探すなら `commons/`、個別コマンド本体を探すなら `sub_commands/` へ進みたいとき。

## Do not read this when

- 個別サブコマンドの詳細仕様や引数だけを確認したいときは、ここではなく該当モジュールや `oracles` を読むべきです。
- 共通モジュールの内部実装だけを追いたいときは、`commons/` 配下の個別ファイルを直接読むべきです。
- サブコマンド本体の業務ロジックだけを追いたいときは、`sub_commands/` 配下の個別モジュールを直接読むべきです。

## hash

- 5f412b0e7bacf526e88617de500f615977289d2fe46e74bcea43802ca60f4489
...
- `conftest.py` は `<cmoc-root>/src` を import path へ追加する共通設定です。
- `test_codex.py`、`test_repo.py`、`test_subcommands.py`、`test_indexing.py` などが、Codex 呼び出し・git 共通処理・サブコマンド制御・INDEX.md 生成を検証します。
- `test_file_naming.py`、`test_report_files.py`、`test_timestamps.py` は補助的な命名・保存・時刻フォーマットの回帰を担います。

## Read this when

- pytest のテスト配置や、このディレクトリにどの種類の回帰テストがあるかを確認したいとき。
- `src` 側の実装変更が既存テストのどこに影響するかを俯瞰したいとき。
- 新しいテストを追加するときに、既存のテスト群の役割分担を把握したいとき。

## Do not read this when

- cmoc の利用手順やサブコマンド仕様だけを確認したいときは、`oracles/app_specs` 側を読むべきです。
- 個別の実装ロジックだけを追いたいときは、`src` 配下の該当モジュールを直接読むべきです。
- テスト全体ではなく、特定の 1 ファイルだけを確認したいときは、この入口ではなくそのテストファイルを直接読むべきです。

## hash

- 856500313080bccfea94e322542379a466eb1d9237629c380339298bfda8b7d9
