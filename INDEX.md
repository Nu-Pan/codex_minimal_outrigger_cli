...
## Summary
- oracle 配下の正本仕様断片を読むための入口。補完の起動判定や副作用境界を扱う doc と、共通基盤・設定・変換・規範文面の正本仕様断片を束ねる src を案内する。

## Read this when
- CLI の補完起動条件や、補完プローブ中に通常実行の前処理・副作用を混ぜてよいかを判断したいときは doc を読む。
- oracle 側で共通に使う設定モデル、変換モデル、状態やルールの正本仕様断片を確認したいときは src を読む。
- 補完時に stdout/stderr へ余計な出力や副作用が入っていないかを確認したいときは doc を読む。

## Do not read this when
- 通常実行時のサブコマンド仕様、状態ファイル仕様、ログ仕様、索引更新仕様そのものだけを確認したいときは、より直接の対象を探す。
- CLI 実装手順、realization 側の実装詳細、テスト構成だけを見たいときはここを読まない。
- 個別の UI、出力、テストの詳細挙動だけを追いたいときは、対応する下位対象を直接読む。

## hash
- 70ed39d7c51cd0efc03fdb9c044c17ead4005e3374abba77a145d2d725736e18
