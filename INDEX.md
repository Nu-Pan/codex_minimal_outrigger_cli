...
## Summary
- `doc` は cmoc の正本仕様を読むための入口で、全体の案内、設計判断の背景、開発時の規約をまとめて確認したいときに進む。
- `src` は oracle 側の共通基盤を読むための入口で、設定・変換・文書化・規範文面の共通部品や、複数 builder にまたがる基礎概念の境界を確認したいときに進む。

## Read this when
- cmoc 全体でどの仕様断片を読むべきか迷っているとき。
- 複数の仕様断片にまたがる責務境界や優先関係を確認したいとき。
- 採用しなかった設計案の背景や不採用理由を確認したいとき。
- oracle 側で共通に使う設定モデル、変換モデル、プレースホルダ展開、Markdown レンダリング、規範文面の組み立てを確認したいとき。
- 複数の builder にまたがる状態、ルール、結果表現の正本仕様断片を確認したいとき。

## Do not read this when
- すでに読むべき個別の正本断片が分かっているとき。
- 特定機能の詳細仕様や実装手順だけを確認したいとき。
- 現行仕様そのものではなく、却下された案の背景が不要なとき。
- CLI の実行手順、realization 側の実装詳細、テスト構成だけを確認したいとき。
- 個別の UI、出力、テストの詳細挙動だけを見たいとき。

## hash
- dec7a67bc57df3c2fbe0408c2b4d38164b8f0b7b043e6018e0c98bf1fc578736
