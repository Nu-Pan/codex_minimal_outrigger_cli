...
- `oracles` 配下の正本仕様断片を一覧し、目的別に読み進めるためのルーティング用ディレクトリの目次です。
- アプリ実行時仕様は `app_specs`、採用しなかった設計案は `considered_alternatives`、開発者向けルールは `dev_rules` に分かれています。
- `app_specs` には `codex exec` 呼び出し規約、ログ、エラー処理、`INDEX.md` 自動生成、利用手順、各サブコマンド仕様への入口がまとまっています。

## Read this when

- `oracles` 配下のどの仕様ファイルから読むべきかを判断したいとき。
- cmoc の実行時仕様、開発ルール、あるいは採用しなかった設計案のどれを参照すべきか切り分けたいとき。
- `cmoc` の利用手順、個別サブコマンド仕様、共通エラー処理、`INDEX.md` メンテナンス仕様を探したいとき。

## Do not read this when

- すでに読むべき個別仕様ファイルが分かっていて、ルーティング情報が不要なとき。
- `README.md`、`AGENTS.md`、`memo` のアクセス制約や編集可否だけを確認したいとき。
- `cmoc` の具体的な実装コードやテストコードの場所だけを調べたいとき。

## hash

- a97d2d7d8a0a21419f722a35d0f65488a5b335d60c4d371413b98090bd06a6ff
