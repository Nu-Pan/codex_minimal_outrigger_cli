...
- この `INDEX.md` は `<work-root>/oracle` 配下のルーティング文書で、`doc/` と `src/` への入口を案内します。
- `doc/` 側は利用手順・共通仕様・採用しなかった設計判断・開発規約・branch モデルの起点をまとめる想定です。
- `src/` 側は `acp/`、`basic/`、`config/` への入口で、共通基盤・設定・パス解決・文書レンダリングの役割分担を整理する起点です。

## Read this when

- `<work-root>/oracle` 配下で、まず `doc/` と `src/` のどちらから読むべきか整理したいとき。
- cmoc の利用方法、共通仕様、branch モデルへの入口をひとまとめに把握したいとき。
- 採用しなかった設計判断、開発規約、共通基盤、設定や文書レンダリングの入口を俯瞰したいとき。
- 下位ディレクトリの `INDEX.md` や個別仕様へ進む前に、この階層の役割分担を確認したいとき。

## Do not read this when

- `<work-root>/oracle` 全体ではなく、`doc/` か `src/` のどちらかの下位文書へ直接進む目的が決まっているとき。
- この階層ではなく、`doc/` 配下の個別仕様や `src/` 配下の個別仕様だけを確認したいとき。
- 開発規約や実装手順ではなく、他のルートの案内文書を探しているとき。

## hash

- bff2db9fe2d9eaaac225acbdf4c5adc218447b1e14ba69ec8e01c10bd8af16ac
