...
- `doc/` では cmoc の利用仕様・共通仕様・開発規約・採用しなかった設計判断を案内します。
- `src/` では実装で参照する共通型・呼び出しパラメータ生成・共通ユーティリティを案内します。

## Read this when

- `oracle` 全体の入口を把握して、どの下位ディレクトリから読むか整理したいとき。
- cmoc の仕様断片を探す前に、`doc/` と `src/` の役割分担を確認したいとき。
- この階層の下位 `INDEX.md` や個別仕様へ進む前に、全体像をつかみたいとき。

## Do not read this when

- 読む対象がすでに `doc/` または `src/` に決まっていて、この階層の入口を経由する必要がないとき。
- 個別の仕様ファイルや実装ファイルを直接確認したいとき。
- `README.md` や `AGENTS.md` など、`oracle` 以外のリポジトリ運用情報だけを確認したいとき。

## hash

- b2334fee688f814f0a3e96fa65b48aa9484928e8c31cd88f40d20a19f3c13627
