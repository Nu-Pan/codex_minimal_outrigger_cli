...
- `src/` は `acp/`、`basic/`、`config/` への入口で、共通基盤・prompt 構成・設定関連の正本仕様をたどる起点です。

## Read this when

- `<cmoc-root>/oracle` 配下の全体構成を先に把握し、`doc/` と `src/` のどちらへ進むべきか整理したいとき。
- cmoc の正本仕様断片と実装の入口を、この階層からまとめて確認したいとき。
- 上位のルーティング文書として、`oracle` の役割分担を短時間で把握したいとき。

## Do not read this when

- すでに読む先が `doc/` か `src/` に決まっていて、この階層の入口説明が不要なとき。
- `oracle` 全体ではなく、特定の正本仕様ファイルや個別実装ファイルへ直接進みたいとき。
- この階層のルーティングではなく、別階層の文書や別目的の情報を探しているとき。

## hash

- f087a24383358bc066eda43f0e86d29beb9c03681858a30a1fbb61441277baac
