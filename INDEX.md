...
## Summary
- cmoc の正本文書群と実装層を目的別に案内する入口。
- 現行仕様・開発ルール・設計判断の背景は `doc` へ、agent call 構築や共通処理の実装確認は `src` へ進む。

## Read this when
- cmoc の正本仕様、開発ルール、設計判断の背景、または実装層の入口を探すとき。
- 現行仕様や開発規則を読むべきか、agent call 構築の実装を読むべきかを判断するとき。

## Do not read this when
- 目的の個別仕様、開発ルール、検討資料、または実装ファイルがすでに特定できており、その対象を直接読めば足りるとき。
- 具体的なアプリケーション挙動、入力契約、agent call の詳細実装だけを確認したいとき。

## hash
- a83b04b9ed0969646a767d45e9e99da52a6a8858d897b7e828105646ab5b50f7
