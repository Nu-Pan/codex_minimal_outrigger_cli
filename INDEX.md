...
## Summary
- `oracle` は、このリポジトリの正本仕様断片をまとめる入口である。個別機能の外部挙動、分岐条件、状態遷移、終了条件、設計上の不採用案を確認したいときに進む。
- `doc` は、アプリ固有の仕様断片を読むための入口である。`app_spec` は個別機能の正本仕様を確認するとき、`branch_model.md` は session/run の branch と worktree の対応を確認するとき、`considered_alternative` は採用しなかった設計案とその理由を確認するとき、`dev_rule` は Python 実装・テストの書き方方針を確認するときに読む。
- この領域は共通 CLI 基盤や横断規約の入口ではない。利用者に見える個別機能の仕様、設計判断、実装記法を絞って読むためのルーティングに使う。

## Read this when
- このアプリの仕様、実装、テストを追加・修正するとき。
- session 作成時の分岐元や最終 merge 先を決める必要があるとき。
- 現行設計に対して、過去の不採用案や実装方針を再確認したいとき。
- Python の実装またはテストを追加・修正し、型ヒント、import、docstring、コメント、ログ、命名の方針を確認したいとき。

## Do not read this when
- 共通の CLI 基盤、ログ規約、エラー処理、索引更新、事前検証などの横断仕様だけを確認したいとき。
- git の一般的な branch 運用や worktree 機能そのものを学びたいだけで、cmoc 固有の対応関係が不要なとき。
- 採用済みの詳細仕様、CLI 入出力、状態ファイル仕様、テスト期待値、実装経路を確認したいとき。
- CLI の具体的なコマンド仕様、出力 schema、保存状態、エラー条件など、利用者に見える個別機能の正本仕様を確認したいとき。

## hash
- 7be7d8876704e9272aa673eb64fa428b9a74232f7f1882c2ce4c96204e41a26a
