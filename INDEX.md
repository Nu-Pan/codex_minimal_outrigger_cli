...
## Summary
- `cmoc` の正本仕様断片を集める上位入口。共通方針、branch/worktree、採用しなかった設計案など、個別機能へ入る前に確認すべき文書群を案内する。
- 実装手順そのものを説明する場ではなく、目的に応じて下位文書を選ぶためのルーティング層として使う。

## Read this when
- `cmoc` の共通方針やサブコマンド横断の背景を先に確認したいとき。
- どの文書を読むべきか迷い、採用済み仕様と見送った設計案のどちらへ進むか切り分けたいとき。
- 個別機能に入る前に、上位の整理だけを押さえたいとき。

## Do not read this when
- 読む対象の個別文書がすでに特定できているなら、ここではなく該当文書を直接読む。
- 単一機能の入出力や状態遷移だけを確認したいとき。
- 実装や一般論だけを知りたいとき。

## hash
- f0db707158588467230e0ee620b02e6003a57ea162cd58a61c36339035d08e56
