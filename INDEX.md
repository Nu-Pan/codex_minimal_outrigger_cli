...
## Summary
- cmoc の oracle 文書と oracle 実装をまとめる領域です。文書は要求・挙動・開発規約の正本を担い、実装コードは agent 向け prompt や呼び出し parameter、その構成に必要な共通処理を担います。
- 文書にはサブコマンドの仕様、oracle と realization の扱い、開発規約、設計判断の記録が含まれます。

## Read this when
- oracle 内の複数領域にまたがる作業で、参照すべき正本文書や実装の所在を見定めるとき。
- cmoc の要求・外部挙動・開発規約を確認または変更するとき。
- agent 向け prompt、policy 文面、呼び出し parameter の構成や関連する共通処理を調べるとき。

## Do not read this when
- 対象の仕様文書や実装がすでに特定できているときは、その対象へ直接進む。
- oracle の内容を確認する必要がない realization の実装・テスト作業では、対応する実装やテストを起点にする。
- 既存のテストと品質検査の実行手順だけを確認するときは、その手順を定める文書を直接読む。

## hash
- b7ee4452c5afb87036cf639dd219184331e7fee7f9c714128f7167fecb1884b9
