...
- `doc/` は cmoc の利用手順・共通仕様・非採用案・開発規約・branch モデルへの入口をまとめ、`src/` は AI 呼び出し基盤・共通基盤・設定基盤への入口を案内します。
- この階層は、cmoc の正本仕様断片を目的別にたどるための最上位の起点です。

## Read this when

- `<cmoc-root>/oracle` 配下の文書を大きく俯瞰して、まず `doc/` と `src/` のどちらに進むべきか整理したいとき。
- cmoc の利用手順・共通仕様・非採用案・開発規約・branch モデル、または AI 呼び出し基盤・共通基盤・設定基盤の入口をまとめて把握したいとき。
- 下位の `INDEX.md` や個別仕様へ進む前に、この階層の役割分担を先に確認したいとき。

## Do not read this when

- `<cmoc-root>/oracle/doc` または `<cmoc-root>/oracle/src` の下位に進む先がすでに決まっていて、この階層の入口を経由する必要がないとき。
- `app_spec/`、`considered_alternative/`、`dev_rule/`、`branch_model.md` のいずれか、または `acp/`、`basic/`、`config/` の個別文書を直接確認したいとき。
- `INDEX.md` の生成ルールではなく、`oracle` 全体の設計方針や個別仕様本文を探しているとき。

## hash

- 85f8b610b8ad7bae67bcefa0bdcd3910d35a9d092b9450ffa74ad410c370b6a5
