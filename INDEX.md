...
## Summary
- `oracle/doc` 配下の個別仕様断片へ進むための入口。CLI の起動前処理、補完、ログ、エラー、状態、実行分離、外部 provider 境界など、利用者に見える振る舞いの断片を探すときに使う。
- `oracle/src` 配下の正本仕様断片を責務別に辿るための入口。共通概念、型、出力契約、prompt 構成、実行条件、設定やパス表現、文書モデル、Markdown 補助を確認したいときに使う。

## Read this when
- アプリ全体の仕様断片について、どの文書へ進んで読むべきか判断したいとき。
- CLI の起動前処理、補完、ログ、エラー、状態、実行分離、外部 provider 境界のうち、個別の振る舞いを実装・修正・検証したいとき。
- `oracle/src` 配下のどの領域を読むべきか、まず入口を絞りたいとき。
- builder 間で共有する概念、型、出力契約、各サブコマンドの prompt や実行条件のどれを確認すべきか整理したいとき。
- agent call 用プロンプトの構成要素や、共通規範の注入位置を確認したいとき。
- 設定、パス表現、文書モデル、Markdown レンダリング補助の基礎を確認したいとき。

## Do not read this when
- oracle file と realization file の一般的な役割分担や編集規則だけを確認したいとき。
- 個別のサブコマンド仕様やデータ構造の細部だけを直接見たいときは、該当する下位文書へ進むほうが適切なとき。
- 個別の実装詳細やテストの中身だけを確認したいとき。
- `oracle/src` 以外の仕様を探しているとき。
- 既に読む対象の下位領域が特定できていて、この入口で再度絞り込みたくないとき。

## hash
- 3a3b8b7135f0d3c875b2f98e8fab6be30160e1b669f40877e21fcd193fa87084
