...
## Summary
- cmoc の正本仕様断片を収める領域。自然言語の設計・規約・判断基準と、AI エージェント呼び出し・共通プロンプト生成・設定・パス表記などの正本実装断片への入口になる。
- 利用者向け CLI 挙動、agent call、ログ、状態管理、branch/worktree モデル、開発時ルール、共通規範プロンプト、Structured Output schema など、realization file を導くための人間意図を確認する起点になる。

## Read this when
- cmoc の実装・テスト前に、正本仕様断片として扱うべき自然言語文書や正本実装断片を探すとき。
- CLI 挙動、サブコマンド、実行前処理、Codex CLI 呼び出し、ログ、エラー処理、状態ファイル、インデクシング、run 隔離に関する仕様の入口を探すとき。
- session fork/join、apply/review、run ごとの branch・commit・worktree、管理 branch の命名や意味を確認したいとき。
- AI エージェント呼び出しの基本構造、機能別の呼び出し方針、prompt 構築、権限、モデル方針、preflight、出力契約を確認したいとき。
- oracle file、realization file、レビュー、INDEX.md エントリー、ファイル読み書きなど、AI に注入する共通規範や標準を確認したいとき。
- 設定既定値、パス表記、ルート概念、規範文書モデル、構造化 Markdown レンダリングなど、複数領域から使われる正本実装断片を確認したいとき。

## Do not read this when
- 既存 realization code の具体的な関数名、内部ロジック、現在のファイル配置、テスト期待値だけを調べたいとき。
- AI エージェント呼び出しや正本仕様断片ではなく、CLI サブコマンドの実行制御、branch 操作、diff 取得、保存処理、表示整形、対象ファイル探索の実装だけを調べたいとき。
- バックエンド固有のプロセス起動、モデル名や reasoning effort 名への変換、結果処理、エラー処理の realization implementation だけを確認したいとき。
- 設定値の正本ではなく、設定ファイルを実際に読み書きする実装だけを探しているとき。
- 特定の個別仕様の所在が既に分かっており、その文書または正本実装断片だけを直接読めば判断できるとき。

## hash
- 32abca05c9a6c5b938d0f0512da3f90b96be15916b29c7887fec703f32bc5a67
