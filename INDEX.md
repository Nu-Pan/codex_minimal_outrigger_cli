...
## Summary
- `oracle` 配下の正本仕様断片への入口。`app_spec` を起点に、CLI 全体にまたがる起動前処理、補完、実行呼び出し、ログ、事前検証、エラー処理、model provider 境界、索引更新、prompt 標準、run 隔離、session 状態、利用手順を確認したいときに進む。
- 個別サブコマンドの仕様ではなく、CLI 全体で共通に効く横断仕様を読むための案内である。
