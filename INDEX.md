...
## Summary
- `test` 配下の共通テスト補助と各 `realization test` の入口を束ねる案内対象。個別の実装本文ではなく、テスト支援の共通化方針や、どのテストがどの責務を担うかを選ぶために読む。
- この配下は、CLI・runtime・builder・prompt まわりの振る舞いを外部挙動として確認するテストと、そのための小さな共通補助からなる。正本仕様そのものではなく、仕様を検証する側をたどるときのルーティング点として使う。

## Read this when
- 特定のサブコマンドや runtime の外部挙動を検証するテストを探したいとき。
- 共通テスト補助がどの責務を持ち、どのテスト群から使われるかを確認したいとき。
- 正本仕様ではなく、そこを検証する realization test の入口を選びたいとき。

## Do not read this when
- 個別機能の正本仕様を確認したいときは、対応する `oracle` 側を読む。
- テスト補助の内部実装そのものを追いたいときは、該当する helper 本文を直接読む。
- `INDEX.md` の書き方やルーティング方針自体を確認したいときは、この配下ではなく上位の案内を読む。

## hash
- 80769724bba18648131b3e006919547ac0f3c4e6e0fcc095a111bc93bf9a2058
