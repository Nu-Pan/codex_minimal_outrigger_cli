...
- `conftest.py` がテスト実行時の import path を調整し、各 `test_*.py` が主要機能の回帰を分担しています。
- この目次は、`tests` 配下の責務を把握して必要なテストへ素早く辿り着くための入口です。
...
- どのテストファイルがどの責務を守っているかを素早く把握したいとき。
- `tests` ディレクトリの入口として、個別テストへ進む前に全体像を整理したいとき。

## Do not read this when

- 本体実装を追いたいだけで、`src` 配下のコードを読むとき。
- `oracles` 側の正本仕様やルーティング方針だけを確認したいとき。
- 個別サブコマンドの仕様や利用手順を知りたいだけで、pytest 回帰の整理が不要なとき。
- `tests` 配下の各ファイルの責務を知る必要がなく、別ディレクトリの仕様だけを見たいとき。

## hash

- 2165d1036d28e82fb7eb6f241aeb30615bd29052b760d970d4eb43bbda64bc97
