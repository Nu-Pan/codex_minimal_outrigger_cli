...
- `oracle` 配下のルーティング文書で、`doc/` と `src/` への入口を案内する。
- `doc/` は利用手順・共通仕様・採用しなかった設計判断・branch モデルの案内、`src/` は共通基盤・設定・パス解決・標準表現・構造化文書レンダリングの案内を担う。
- 下位の `INDEX.md` や個別仕様へ進む前の起点として使う。
...
- `app_spec/`、`considered_alternative/`、`dev_rule/`、`branch_model.md` のうち、読む対象がすでに決まっているとき。
- この階層ではなく、下位ディレクトリの `INDEX.md` や個別仕様だけを確認したいとき。
- `README.md` や `AGENTS.md` など、`doc` 以外のリポジトリ運用ルールを確認したいとき。

## hash

- 32a31e4e16680b83cf8b282ebbf486b195e8ac06ed8179eb15007714856f9129
