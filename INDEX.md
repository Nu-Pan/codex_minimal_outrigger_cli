...
## Summary
- cmoc の人間所有の正本仕様文書と、文書から委譲された prompt・構築・agent call の正確な定義を収め、意味仕様と実行定義の責務を分けている。
- 機能やサブコマンドの要件、oracle と realization の責務、Git と run の運用、横断的な動作、開発・検証規則、検討した代替案を扱う。

## Read this when
- cmoc の仕様変更や解釈で、要求・判断基準・優先関係の正本や担当領域を特定するとき。
- oracle と realization の責務やファイル分類、文書と実行定義の委譲関係を確認するとき。
- prompt の正確な文面や構築方法、schema、agent call の設定が作業の根拠になるとき。

## Do not read this when
- 対象の機能と該当する個別仕様が分かっており、その仕様だけを確認すればよいときは、個別仕様へ直接進む。
- 仕様が特定済みの実装・テスト作業では、関連する仕様と成果物へ直接進めばよいとき。
- 確認対象の prompt や agent call の定義が特定済みで、その定義だけで足りるときは、委譲先の定義へ直接進む。

## hash
- 1b2c69ce9167d4f774a9533618dbb47932e4af8bfa364203d7e5f8117f8bb9ab
