...
## Summary
- 正本仕様断片を扱う領域への入口。自然言語文書、正本実装断片、正本テスト、開発補助手順など、人間意図として実装差を避けたい内容を確認するための下位領域を束ねる。
- AI 呼び出し仕様、共通プロンプト、パスモデル、設定既定値、CLI 挙動、branch/worktree モデル、作業隔離、インデクシング、開発時の横断ルールなど、実装の根拠となる仕様断片へ進むための起点になる。

## Read this when
- cmoc の実装やテストを変更する前に、根拠となる正本仕様断片の所在を絞りたいとき。
- CLI 挙動、状態管理、ログ、作業隔離、agent call、Structured Output、共通規範プロンプトなど、利用者向け挙動や AI 呼び出し仕様に関わる人間意図を探すとき。
- パス表記、ルート概念、設定既定値、モデル方針、preflight、レビュー、差分要約、索引エントリー生成など、複数領域にまたがる正本実装断片を確認したいとき。
- 開発者向け補助手順や、現行設計で採用しなかった代替案と不採用理由を確認したいとき。
- 自然言語仕様、正本実装断片、補助手順のどの下位領域へ進むべきかを判断したいとき。

## Do not read this when
- 実装ファイルの現在の関数構造、内部 helper、既存テストだけを直接調べたいとき。
- 正本仕様断片ではなく、利用者向けではない一時生成物、実行ログ、キャッシュ、作業メモを探しているとき。
- Codex CLI、git、ollama、permission profile など外部ツール一般の使い方だけを調べたいとき。
- oracle file と realization file の一般的な役割分担や品質基準だけを確認したいとき。

## hash
- 691ae227e5fa26ed56f28902d9493303fdb98bd08345628ff8ba8ab409646eae
