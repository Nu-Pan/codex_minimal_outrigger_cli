...
## Summary
- `oracle` 配下の正本仕様断片全体への入口。ここでは、`cmoc` の文書仕様と実装仕様をどちらへ進むべきかをまず切り分けるために使う。
- この階層は、branch / run / session の関係、使用方法、設計方針、各種アプリ仕様、共通標準、採用しなかった案を横断して確認したいときに読む。

## Read this when
- `oracle` 配下のどの正本仕様断片を読むべきかまだ分からないとき。
- `cmoc` の動作境界、作業の流れ、設計上の前提、共通ルールを先に確認したいとき。
- 個別の文書へ直接入る前に、文書群の役割分担を見たいとき。

## Do not read this when
- 読むべき個別文書がすでに分かっているときは、ここではなくその文書を直接読む。
- 内部実装の手順や関数分割だけを見たいとき。
- 正本仕様の更新ルールや案内文の生成方法だけを調べたいとき。
