...
- `cmoc` の正本仕様断片を集めた `oracles` ディレクトリの入口です。
- この目次からは、アプリ仕様をまとめた `app_specs`、開発ルールをまとめた `dev_rules`、採用しなかった設計案をまとめた `considered_alternatives` へ進めます。
- `oracles` 配下の非 `INDEX.md` ファイルは仕様断片として扱われ、個別の内容はそれぞれのディレクトリの目次から辿る前提です。

## Read this when

- `cmoc` の仕様断片をどのディレクトリから辿るべきか整理したいとき。
- `app_specs`、`dev_rules`、`considered_alternatives` のどれへ進むべきか判断したいとき。
- `oracles` 配下のファイルが正本仕様断片としてどう扱われるか、全体の入口から確認したいとき。

## Do not read this when

- `cmoc` の個別サブコマンド仕様だけを確認したいときは、この入口ではなく各ディレクトリ配下の `INDEX.md` や個別仕様ファイルを直接読むべきです。
- `cmoc` の実装コードやテストコードだけで用が足りるときは、このディレクトリの目次情報を読む必要はありません。
- `oracles` 配下の特定ファイル名がすでに分かっていて、その本文だけを確認したいときは、この入口ではなく該当ファイルへ直接進むべきです。

## hash

- 5a657cd8aa5175546ea7a0d4da20786b1b5132eb8380307bd3841da57cd57dda
