...
- `src` は cmoc 本体の実装が入るディレクトリで、CLI の入口と共通処理、サブコマンド実装をまとめています。
- `main.py` は CLI のエントリーポイント、`commons` は共有基盤、`sub_commands` は各サブコマンドの実装入口です。
- この目次は、実装ファイル群の配置を素早く切り分けて、必要なコードへ最短でたどるための入口です。

## Read this when

- cmoc の実装全体で、エントリーポイント・共通基盤・サブコマンド実装の配置を整理したいとき。
- `src/main.py`、`src/commons`、`src/sub_commands` のどこに何があるかを素早く把握したいとき。
- 実装やテストを始める前に、`src` 配下の役割分担を俯瞰したいとき。

## Do not read this when

- 個別サブコマンドの引数や状態遷移だけを確認したいときは、`src/sub_commands` 配下の該当モジュールを直接読むべきです。
- 共通基盤の詳細だけを追いたいときは、`src/commons` 配下の個別モジュールを直接読むべきです。
- `oracles` 側の正本仕様や `INDEX.md` の生成ルールだけを確認したいときは、このディレクトリを読む必要はありません。
...
- `tests` 配下の pytest 回帰テストを集約した入口で、`src` 実装の振る舞いを守る。
- `conftest.py` の共通設定と、`codex` 呼び出し、`INDEX.md` 保守、リポジトリ共通処理、サブコマンド、タイムスタンプや報告ファイルの検証観点を案内する。
- 個別テストファイルへ進む前に、どの機能の回帰を確認するかを素早く切り分けるための目次である。

## Read this when

- `tests` 配下のどのファイルを読むべきか整理したいとき。
- `pytest` の共通設定や各回帰テストの役割を把握したいとき。
- `src` の変更に対して、対応するテスト観点を確認したいとき。

## Do not read this when

- `oracles` の正本仕様や `INDEX.md` 生成ルールを確認したいとき。
- 個別の実装ロジックや `src` 側の本体仕様だけを追いたいとき。
- テストではなく、CLI の利用手順や開発ルールだけを確認したいとき。
