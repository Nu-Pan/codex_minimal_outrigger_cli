...
## Summary
- AI コーディングエージェント呼び出し時に渡す基本パラメータ群を定義する実装。論理的なモデルクラス、Reasoning effort、ファイルアクセスモードの列挙と、それらにプロンプト本文および Structured Output schema の有無を組み合わせた不変データ構造を扱う。
- バックエンド固有のモデル名や実ファイルアクセス設定へ解決する前段階の、cmoc 内部で使う抽象的な呼び出し条件の入口となる。

## Read this when
- エージェント呼び出しに渡すパラメータの構造、型、必須項目を確認したいとき。
- cmoc 上で選択できる論理的なモデル種別、Reasoning effort、ファイルアクセスモードの候補を確認したいとき。
- Structured Output を要求する呼び出しと要求しない呼び出しで、schema パスをどう表現するか確認したいとき。
- バックエンド実装へ渡す前の抽象的な呼び出し条件を組み立てる処理を実装・変更するとき。

## Do not read this when
- 具体的なモデル名、API パラメータ、サンドボックス設定など、バックエンドが実際に受理する値への解決処理を確認したいとき。
- プロンプト本文の生成規則や内容の組み立て方を確認したいとき。
- Structured Output schema ファイル自体の内容や schema の検証仕様を確認したいとき。
- エージェント呼び出しの実行、プロセス起動、結果取得、エラー処理の流れを確認したいとき。

## hash
- 007ad67d524410a56ffdca61c75816daba79432f33a8715af2c5ca40e4110840
