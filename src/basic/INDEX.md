...
## Summary
- `basic.acp` から ACP 関連の型を公開し、正本型を複製せずに既存の参照経路を維持したいときに読む。再公開だけを担う薄い入口で、型定義そのものや利用側の実装はここでは扱わない。

## Read this when
- `basic.acp` 経由で ACP 型を使う公開面を確認したいとき。
- 正本側の型をそのまま見せつつ、利用側の import 経路を変えたくないとき。
- 再公開されている型名の一覧と、この層が維持している互換性の境界を確認したいとき。

## Do not read this when
- ACP 型の定義本体や意味を確認したいときは、再公開先の正本を読む。
- `basic.acp` 以外の実装や機能の入口を探しているとき。
- この層を経由せずに公開面を整理する設計変更を検討しているときは、呼び出し側と正本側を直接確認する。
