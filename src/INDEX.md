...
## Summary
- cmoc の実行基盤で複数モジュールから共有される runtime helper 群のまとまり。ここでは共有補助の入口と責務の境界だけを示し、個別の入出力や失敗時挙動は下位要素で確認する。
- 実行前後の共通基盤、実行結果コンテナ、共有ロギング、パス解決、Git/state/config/エラー処理のように、複数の上位機能から参照される処理を確認したいときの入口。

## Read this when
- 複数のサブコマンドや runtime helper から使う共通処理の配置場所や責務境界を確認したいとき。
- 実行基盤のうち、パス解決、状態管理、ログ、結果型、Git 依存、エラー整形、preflight のどれへ進むべきかを切り分けたいとき。
- 共有 helper 群の中で、どの対象が必要かを先に絞ってから下位の本文へ進みたいとき。

## Do not read this when
- 特定の helper の実装、入出力、失敗時挙動を確認したいとき。この入口ではなく該当ファイルを直接読む。
- CLI コマンド固有の業務ロジックやテストだけを追いたいとき。共有 runtime helper ではなく、より直接その責務を持つ対象へ進む。
- 人間向けの正本仕様そのものを確認したいとき。この領域は実装側の共有基盤であり、仕様断片の本文ではない。

## hash
- 659c321ce855fbc083d61aae3dda3a00f55add34afb4dcfa54ead8a76d7a4e9c
