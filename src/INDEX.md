...
## Summary
- realization implementation 側で、正本側の basic 関連概念を既存公開面へ再公開する互換層をまとめる領域。ACP 関連型、path model、構造化ドキュメントなどを複製せず、既存の basic 参照を維持するための薄い入口群として位置づけられる。
- 主な責務は実体ロジックの実装ではなく、正本側または実体側の定義・実装への import 経路を保つことにある。互換入口を残す理由や削除条件を確認するための入口として使う。

## Read this when
- 既存コードや利用者向け公開面に残る basic 参照の互換維持、移行、削除条件を確認したいとき。
- ACP 関連型、path model、構造化ドキュメント関連の公開名が realization implementation 側でどの互換境界から提供されているかを確認したいとき。
- 正本側の型や実装を複製せず再公開している理由、または各互換入口に独自ロジックがあるかを切り分けたいとき。
- oracle src 側の basic 関連実装と realization 側または利用者向け公開面との import 経路の関係を確認したいとき。

## Do not read this when
- ACP 関連型そのものの定義、列挙値、データ構造、生成ロジック、変換処理を確認したいとき。その場合は正本側の型定義や責務を持つ実装側を直接読む。
- path 解決ロジック本体、root 判定、path 変換処理の仕様や挙動を確認したいとき。この領域は再公開入口であり、実装本体ではない。
- 構造化ドキュメントの具体的な型、関数、変換規則、検証ロジックを調べたいとき。その場合は正本側の構造化ドキュメント実装を読む。
- CLI 挙動、テスト挙動、INDEX.md エントリー生成規則、oracle file と realization file の基本概念など、basic 互換公開面と直接関係しない作業をしたいとき。

## hash
- d3a1cc1fff105a047d84f0e950c5faaeebc64b659f257101dbd6211ed889e147
