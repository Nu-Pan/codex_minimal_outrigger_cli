...
## Summary
- cmoc 実行時の共通処理を集めた共有 helper 群の入口で、CLI 実行制御、Codex 起動、設定、git、path、ログ、状態、エラー、Ollama、indexing などの実行基盤をまとめて扱う。
- この領域自体は共有 runtime の境界を示すだけで、個別の振る舞いは下位の各モジュールで確認する。

## Read this when
- cmoc の実行時に複数モジュールから使う共通 helper の配置場所や公開入口を確認したいとき。
- 実行基盤のうち、CLI ライフサイクル、Codex 起動、設定変換、git 操作、path 解決、ログ、状態管理、内容 hash、Ollama、indexing のどれかへ進む前に、この共有領域の責務を把握したいとき。

## Do not read this when
- 特定 helper の実装、入出力、失敗時挙動を確認したいときは、この領域ではなく該当する下位モジュールを読む。
- CLI コマンド固有の業務処理やテスト固有の仕様を追いたいときは、共有 runtime ではなく、より直接その責務を持つ対象へ進む。

## hash
- 3014ebeff1c645c27c50b5d3b099dc79fa4236d3119804bcacc2c822388b7e92
