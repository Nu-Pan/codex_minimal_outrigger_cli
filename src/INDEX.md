...
## Summary
- cmoc の実行時に複数箇所から共有される補助機能の集約領域。個別の挙動ではなく、共有 runtime helper 群への入口と責務境界を確認するために読む。
- 配下には、CLI 実行・設定・パス・git・ログ・状態・エラー・Ollama・Codex 実行制御など、横断的に再利用される runtime 実装が並ぶ。具体的な入出力や失敗時挙動は各担当モジュールを読む。

## Read this when
- 複数モジュールから使う共通 helper の配置場所や、runtime 共通 API の公開入口を確認したいとき。
- 実行時の root/path 解決、git 操作、ログ、状態、設定、エラー処理、Codex/Ollama 関連の共通処理のうち、どの責務がこの領域にあるかを見分けたいとき。
- 下位の runtime helper を読む前に、この領域が共有実装のまとまりであることを確認したいとき。

## Do not read this when
- 特定 helper の詳細な入出力、失敗時挙動、永続化形式を確認したいとき。該当する下位モジュールを直接読む。
- CLI サブコマンド固有の処理や利用者向け文言だけを調べたいとき。より直接その責務を持つ command 層へ進む。
- 個別の runtime helper を新規実装したいだけで、共有 API の集約境界を確認する必要がないとき。

## hash
- a246c9d53087742a720a7d4504872205666ef55a1d1132d74473244947eb8a1b
...
## Summary
- `src/sub_commands` は CLI サブコマンド群の入口で、`apply`・`review`・`session`・`tui` などの責務分担を読む前に、この階層でどの実装へ進むべきかを判断するための対象である。
- 各配下は実行フロー、状態遷移、レビュー処理、TUI 起動など役割が分かれているため、目的に応じて読む先を切り替える。

## Read this when
- サブコマンド群の責務分担や、どの実装を読めばよいかを切り分けたいとき。
- サブコマンド実行の入口、状態遷移、作業用 worktree/branch、レビュー、TUI 起動のどれかを変更したいとき。
- パッケージ入口の説明だけで足りるか、個別サブコマンド実装へ進むべきかを判断したいとき。

## Do not read this when
- 具体的な実行フローや git/worktree 操作を追いたいときは、該当サブコマンドの本文を直接読む。
- 出力形式やレポート内容だけを確認したいときは、report 生成側や対応する下位実装を直接読む。
- 共通 CLI runtime や設定ロードの詳細を調べたいときは、この階層ではなく別の担当箇所を読む。

## hash
- 0d4c073b2603b19ce3d549ac0e78b313b1d3411d9bd9ea1e3f9e4af8edda608f
