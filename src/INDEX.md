...
## Summary
- AI エージェント呼び出しに渡す完全なプロンプト、補助文脈、読み書き制約、モデル・推論量、Structured Output schema、返却契約を組み立てる実装領域。仕様レビュー、仕様と実装の照合、所見適用、変更要約、競合解消、TUI 実行前解決、ルーティング文書用エントリー生成など、各工程で AI に何を読ませ、どの制約で何を返させるかを確認する入口になる。
- プロンプトを構成する基本文書、ファイルアクセス規則、ルーティング規則、仕様ファイルと編集対象ファイルの扱い、各種標準文書、レビュー基準、INDEX.md エントリー基準を構造化文書として生成し、必要な基準の追加や用語・root token 置換を行って agent 向け文書列へまとめる責務を持つ。
- サブコマンドや TUI の実行順序そのものではなく、AI 呼び出しの入力文脈と出力 schema を追うための領域。対象ファイル、仕様本文、既知所見、差分、競合対象、元プロンプトなどが、agent への role、goal、補助 prompt、ファイルアクセスモード、schema にどう反映されるかを把握できる。

## Read this when
- AI agent に渡される role、goal、補助 prompt、参照コンテキスト、読み書き制約、モデル種別、推論量、Structured Output schema を確認または変更したいとき。
- 仕様レビュー、所見の列挙・検証・採否・マージ、仕様と実装の照合、所見適用、差分要約、競合解消、TUI 実行前のパラメータ解決、INDEX.md エントリー生成などで、AI に何を判断させ何を返させるかを実装・テストしたいとき。
- 仕様ファイルだけを読ませる、編集対象ファイルへの書き込みを許可する、読み取り専用にする、競合解消時の例外的な編集条件を指定するなど、agent 作業制限の組み立てを追いたいとき。
- ファイルアクセス規則、ルーティング規則、仕様ファイルと編集対象ファイルの基本概念、仕様文書の記述基準、編集対象ファイルの保守基準、仕様レビュー基準、仕様と実装の照合基準、INDEX.md エントリー基準を agent 向けプロンプトとしてどう生成するか確認したいとき。
- 新しい AI 呼び出し用途や agent 向け基準文書を追加し、既存の完全プロンプト生成、基準フラグの依存関係、補助文脈、schema ファイルとの対応に合わせたいとき。

## Do not read this when
- CLI 引数解析、サブコマンド全体の制御フロー、fork 作成、branch 操作、merge 実行、差分適用、永続状態の保存、表示、集計など、AI 呼び出しパラメータ構築の外側にある orchestration を調べたいとき。
- 仕様ファイル、編集対象ファイル、レビュー基準、INDEX.md 運用規則などの正本仕様や基準本文そのものを確認したいだけのとき。
- agent 呼び出し共通型、モデル種別、推論量、ファイルアクセスモード、構造化文書表現、markdown rendering、パス解決、root token 定義などの共通基盤そのものを確認したいとき。
- 実際の変更対象ファイルの中身、個別差分の検出、変更カテゴリ分類、conflict marker の検出や解消アルゴリズム、TUI の表示・対話・入力取得など、AI に渡す prompt ではなくプロダクト処理本体を調べたいとき。
- 個別の INDEX.md エントリー本文を作成・評価したいだけで、エントリー生成用の prompt、schema、agent 呼び出しパラメータの実装を確認する必要がないとき。

## hash
- f5aa5b53769d708185d7c84a31d34321648b9e0580547705d9791f800ec8c76b
