...
## Summary
- `src/commons` 配下の共通基盤を束ねる領域で、runtime や subcommand から横断的に使う小さな共通部品への入口を案内する。
- 個別の共通機能そのものは下位要素に分かれているため、この対象では「どの共通基盤へ進むか」を判断するためのルーティング情報だけを持つ。

## Read this when
- runtime や複数の処理系から使う共通部品の配置先を探したいとき。
- 共通 helper 群の下位要素へ進む前に、この領域が共通基盤のまとまりであることを確認したいとき。

## Do not read this when
- 特定 helper の実装、入出力、失敗時挙動を確認したいときは、該当する下位要素を直接読む。
- CLI 固有の処理やサブコマンド固有の業務ロジックを追いたいときは、より直接その責務を持つ対象へ進む。

## hash
- dba430947aa6a4dc14e664fdcea303ff1d0919e1f42e0710bfe71b295a09b808
