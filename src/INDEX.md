...
## Summary
- cmoc の共通 runtime helper を集約する commons パッケージ。CLI 実行、Codex 呼び出し、設定・状態・パス管理、Git、ログ、結果型、エラー処理、INDEX 更新、editing run lifecycle など、上位実装が横断的に利用する runtime API と個別実装を提供する。

## Read this when
- commons の runtime API 公開入口や、複数の runtime 機能にまたがる依存関係を確認するとき
- Codex exec/TUI、CLI lifecycle、設定・状態・パス・Git・ログ、INDEX 更新、editing run の実装を変更または調査するとき
- 特定の runtime helper の実装詳細を確認するときは、該当する個別 runtime モジュールを直接読む

## Do not read this when
- 特定サブコマンド固有の業務フローだけを調査するとき
- 利用者向け仕様や正本仕様、Structured Output schema を確認するときは、対応する oracle file を直接読む
- 単一領域の実装詳細だけを確認する場合に、commons 全体を読む必要がないとき

## hash
- db896d830850f8bd12b7e4e3e79da3cd294cf207bb61170c058ef212f8fd4278
...
## Summary
- CLI サブコマンド実装をまとめるディレクトリ。doctor、indexing、oracle、realization、run、session、tui などの個別実装および関連パッケージへ進むための入口を提供する。apply と review は現在実装本文がない。

## Read this when
- CLI サブコマンド全体の実装構成や、対象サブコマンドの実行入口を選ぶとき。
- doctor、indexing、oracle、realization、run、session、tui の実装を調査・変更するとき。
- 個別サブコマンドの下位実装へ進む前に、該当する実装ファイルまたはパッケージを特定するとき。

## Do not read this when
- CLI 共通ランタイム、共通 Git 操作、INDEX 更新処理、レビュー判定など、サブコマンド固有実装より下位または共通の責務だけを調査するとき。
- 特定サブコマンドの詳細処理が明確な場合は、このディレクトリ全体ではなく該当する実装ファイルや下位パッケージを直接読む。
- apply または review の実装が未追加の状態で、それらの具体的な処理内容を確認しようとするとき。

## hash
- 8d13942de07f509ae8682a1eb40e91c28e9af369723edfc053ea122964e428cb
