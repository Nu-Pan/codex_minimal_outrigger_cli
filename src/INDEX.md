...
- `src/sub_commands` は、cmoc のサブコマンド実装をまとめるルーティング用ディレクトリです。
- `init`、`branch`、`apply`、`eval-oracles`、`merge` の各本体実装への入口を案内します。
- `__init__.py` はこの配下がサブコマンド実装パッケージであることを示すだけの補助ファイルです。

## Read this when

- `cmoc` の各サブコマンドの実装ファイルがどこにあるか確認したいとき。
- `init`、`branch`、`apply`、`eval-oracles`、`merge` のどのモジュールを読むべきか判断したいとき。
- サブコマンド実装パッケージの境界や、各ファイルの役割をざっくり把握したいとき。

## Do not read this when

- 共通ユーティリティや `commons` 側の実装だけを調べたいとき。
- `cmoc` 全体の開発規約、設計規約、テスト規約だけを確認したいとき。
- 各サブコマンドの正本仕様断片を読みたいだけで、実装ファイルの目次は不要なとき。

## hash

- b80ac310bd32e18d55b9210224bdc2a314b3b02d153ecc960dc5c14bd89bf12a
