...
## Summary
- `commons` 配下の実行時共通基盤をまとめる領域の入口。`runtime_*` 系の処理を横断して使うときにここから下位モジュールへ進む。
- 個別の実装詳細ではなく、Codex 実行、設定・状態・ログ・パス・Git・エラー・結果型の共通責務がどこにあるかを案内する。
- 共有 helper の境界だけを確認したい場合に読む対象であり、各 helper の入出力や失敗時挙動は下位要素で確認する。

## Read this when
- 実行時共通処理の入口を探したいとき。
- 設定、状態、ログ、パス、Git、エラー、結果型のどれがどの helper にあるかを見分けたいとき。
- 複数モジュールから再利用される runtime helper の配置場所を確認したいとき。

## Do not read this when
- 特定 helper の具体的な実装や失敗時挙動を知りたいとき。この対象ではなく該当する下位要素を読む。
- CLI コマンド固有の業務ロジックやテストを追いたいとき。共有 runtime helper ではなく、より直接その責務を持つ対象へ進む。
- run/work root や保存先の共通基盤ではなく、その上にある個別コマンドの入出力だけを見たいとき。

## hash
- 45108b23f28f5e618e145b04842f6f7a3d28655a7ea1f79deb72853d8c24240d
