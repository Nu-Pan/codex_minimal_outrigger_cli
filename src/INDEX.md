...
## Summary
- cmoc の共通 runtime helper を集約する commons パッケージ。CLI 実行 lifecycle、Codex 呼び出し、設定・状態・Git・パス・ログ・エラー・feedback・INDEX 更新など、複数の runtime 機能から再利用される公開 API と個別実装への入口を提供する。

## Read this when
- 複数の runtime 機能にまたがる共通 API、実行 lifecycle、状態管理、Codex 連携、Git 操作、パス、ログ、feedback、INDEX 更新の実装箇所を探すとき
- commons パッケージの公開入口や、対象機能に対応する個別 runtime module へのルーティングを確認するとき

## Do not read this when
- 特定の runtime helper のアルゴリズム、永続化形式、CLI サブコマンド固有の挙動、または正本仕様だけを確認したいとき
- 対象機能を直接担当する個別 runtime module や oracle 文書が明らかな場合

## hash
- 2dfe22d9795cb5542b7b52b592f71a85bce6829dc71167b1c027848119163527
...
## Summary
- CLI サブコマンドの実装をまとめるディレクトリ。doctor、feedback、indexing、oracle、realization、run、session、tui などの各サブコマンド入口と、その実行フローを確認するための上位ルーティング入口となる。apply と review は現在実装がなく、将来の実装配置先として示されている。

## Read this when
- 複数の CLI サブコマンドにまたがる実装構成や、目的のサブコマンド実装への入口を確認するとき
- サブコマンドの実行経路、ライフサイクル、状態管理、レポート生成などを横断して調査するとき

## Do not read this when
- 特定サブコマンドの詳細実装だけを調査する場合は、対応する下位実装を直接読む
- サブコマンド共通ランタイム、正本仕様、入力契約や Structured Output schema の詳細だけを確認する場合は、それぞれの対応する実装・仕様を直接読む

## hash
- 8a6c6f4a36c6df448a8a5b83d90a37507519196c2cb21ca585a8e1790d3b3279
