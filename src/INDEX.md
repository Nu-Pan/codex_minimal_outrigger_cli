...
## Summary
- `commons` 配下の実行基盤ヘルパー群を束ねる入口。root 解決、ログ、結果型、設定、state、Git、Codex 起動、前処理、実行結果整形など、複数の上位モジュールから共有される横断責務の案内点になる。
- この対象そのものは個別 helper の実装ではなく、共有 runtime 基盤のまとまりを読むかどうかを判断するためのルーティング入口として使う。

## Read this when
- `commons` 配下の共有 runtime helper 群のどこを読みに行くべきか判断したいとき。
- サブコマンド実行の前後で共通に使う基盤処理を探していて、個別 helper への入口を絞り込みたいとき。
- root 解決、path/時間表記、ログ、結果コンテナ、設定変換、state 管理、Git 判定、Codex 起動、前処理、エラー整形のような横断的責務を確認したいとき。

## Do not read this when
- 特定の helper の入出力、失敗時挙動、内部分岐を確認したいとき。この対象ではなく、該当する下位要素を直接読む。
- CLI コマンド固有の業務ロジックやテスト固有の処理を追いたいとき。共有基盤ではなく、より直接その責務を持つ対象へ進む。
- 共有基盤の入口だけを確認したいのであれば十分だが、実装詳細や個別の仕様確認のためにここを深掘りする必要はない。

## hash
- e58873cceba3da99f9668de9e3d7b2aaa448b8d4d453f12395690e391eab7203
