...
## Summary
- `acp` 名前空間の公開入口をまとめる互換層。実体の実装へ進む前に、`acp.*` 参照をどこまで維持するか、どこへ移すかを判断するために読む。
- この対象は実装本体ではなく、`acp.builder` など下位の互換入口へ進むための起点として位置づける。

## Read this when
- `acp.*` の既存 import を残すべきか、別 module へ移すべきかを判断したいとき。
- `acp` 配下のどの公開面が互換維持のために残っているかを確認したいとき。
- `acp` 名前空間の下位対象へ進む前に、入口の責務境界を確認したいとき。

## Do not read this when
- 実装本体や生成ロジックを調べたいとき。ここは入口であり、詳細は下位の実体へ進む。
- 新しい機能や API を追加する場所を探しているとき。ここは互換維持用で、機能追加の起点ではない。
- `acp` の公開面をすでに移行済みで、この入口の維持可否だけを確認する必要がないとき。

## hash
- e0b35c791897b303fb5172403d98d366096e9a0ab935cf2a7112a61a45553d8a
...
## Summary
- `commons` 配下の実行時共通 helper を束ねる入口。個別 helper の詳細ではなく、実行基盤全体で共有される責務の切り分け先を案内する。
- この領域は Codex 実行、設定・状態・ログ・パス・Git・エラー処理など、複数モジュールで再利用される runtime 基盤のまとまりとして読む。

## Read this when
- 実行時共通 helper の配置場所や入口を確認したいとき。
- 共有 helper 群へ進む前に、この領域が runtime helper のまとまりであることを確認したいとき。

## Do not read this when
- 特定 helper の実装、入出力、失敗時挙動を確認したいとき。この対象ではなく該当する下位要素を読む。
- CLI コマンド固有の処理やテスト固有の処理を調べたいとき。共有 runtime helper ではなく、より直接その責務を持つ対象へ進む。

## hash
- 950e29652cf06f736e4e7358ad25631baf56570d6892b420dcc76a9798c145b7
