...
## Summary
- `commons` 配下の runtime 共通基盤を集約する領域の入口。外部コマンド実行、Codex 起動、設定・状態・ログ・Git・パス・結果型・エラー変換など、複数モジュールから再利用される補助処理をまとめる。
- この領域は個別サブコマンドの業務ロジックではなく、実行基盤と共通 helper の責務境界を示す。具体的な挙動は下位の各本文を読む前提で、この入口ではどの共通基盤へ進むべきかを判断する。

## Read this when
- cmoc の実行基盤や共通 helper の配置・責務・入口を確認したいとき。
- 複数モジュールから共有される runtime 処理のうち、どの共通層を読むべきかを切り分けたいとき。
- Codex 実行、設定、状態、ログ、パス、Git、エラー、結果型のうち、実装全体の土台を先に把握したいとき。

## Do not read this when
- 特定 helper の入力・出力・失敗時挙動を確認したいときは、この入口ではなく該当する下位本文を読む。
- 個別サブコマンドの入出力や業務フローを追いたいときは、共通 runtime ではなくその責務を持つ対象へ進む。
- 正本仕様そのものを確認したいだけなら、この実装入口ではなく対応する oracle 側の文書を読む。

## hash
- 9a234472ecb8dc741ab0fc4fb3dbb994e661cedf00242b6cd305c7765e1ab5c5
