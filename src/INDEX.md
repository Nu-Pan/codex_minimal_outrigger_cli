...
## Summary
- `src/commons` 配下の共通 runtime helper 群への入口。個別の実装ではなく、共有される基盤や横断責務の所在を確認したいときにここを起点にする。
- この領域は、設定・状態・Git・パス・ログ・Codex 実行・前処理など、複数サブコマンドから再利用される共通処理をまとめる。個別の振る舞いは下位の各モジュール本文で確認する。

## Read this when
- 複数モジュールから使う共通 helper の配置場所や入口を確認したいとき。
- runtime helper 群の役割分担を見て、設定・状態・Git・パス・ログ・実行制御のどこへ進むべきか判断したいとき。
- 実行時の共通基盤をまたぐ変更をしたいが、まず対象の責務境界を絞り込みたいとき。

## Do not read this when
- 特定の helper の入出力や失敗時挙動を知りたいときは、この入口ではなく該当モジュール本文を読む。
- CLI コマンド固有の手順やテスト固有の確認をしたいときは、共通 helper 群ではなく直接その責務を持つ対象へ進む。
- 設定・状態・Git・パス・ログのうち 1 つだけを調べたいときは、横断入口ではなく専用モジュールを直接読む。

## hash
- 8945be2ceb1228a975111ec8b98edb58ce366b4fd7dec3c075442e2183ba82a8
