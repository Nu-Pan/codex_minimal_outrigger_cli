...
## Summary
- `acp` 系の互換入口を束ねる層。正本側へつながる再公開や委譲を追いたいときの起点になる。
- この階層は機能本体ではなく、公開 import 面と互換維持の責務を持つため、実装内容そのものよりも入口の存続可否を判断するときに読む。

## Read this when
- `acp` から正本側の実体へ移す途中で、互換入口を残す必要があるか判断したいとき。
- 旧名の公開面から正本側へどうつながっているか、どの入口が維持されているかを確認したいとき。
- 公開 import 面を整理するときに、どの参照を残しどれを消すかを決めたいとき。

## Do not read this when
- 機能本体や生成処理の中身を調べたいとき。ここは互換入口なので、実体側へ進む。
- `acp` の新機能や新しい API の追加場所を探しているとき。ここは互換維持専用で、機能追加の入口ではない。
- すでに公開面から `acp` 参照を消す方針が確定していて、互換入口の存続可否を検討しないとき。

## hash
- 76243aa69fcb9e1f63cd80a549fa69e700d3bef501daa71ef7611d42317aa8b3
...
## Summary
- cmoc の実行時共通処理をまとめる共有ヘルパー領域の入口。個別の責務は下位モジュールに分かれており、この階層は runtime helper 群の集約点として扱う。
- 複数の実行経路で再利用される CLI 実行、Codex 起動、設定、git、path、logging、state、結果型などの共通基盤を参照するときのルーティング対象。
...
## hash
- 334b50f1609d8529796cc7c043b436a249157f80faa42dc7110b0426b6eb9a75
...
## Summary
- `src/sub_commands` は cmoc の各サブコマンド実装へ進むための入口で、`apply`・`review`・`session`・`tui` などの責務境界を案内する対象である。
- ここでは個別処理の本体ではなく、どのサブコマンド群を読むべきかを切り分けることに重点がある。

## Read this when
- サブコマンド全体の責務分担や、どの実装へ進むべきかを切り分けたいとき。
- `apply`・`review`・`session`・`tui` など、個別サブコマンドの入口だけを把握したいとき。
- 共通 CLI ルーティングと、個別サブコマンド実装のどちらを先に読むべきか判断したいとき。

## Do not read this when
- 個別サブコマンドの具体的な実行フロー、状態遷移、出力形式を追いたいときは、該当サブコマンドの本文を直接読む。
- CLI runtime の共通規約や設定ロードの詳細を確認したいときは、共通基盤側を読む。
- サブコマンド間の共通処理ではなく、特定のサブコマンド固有の振る舞いだけを確認したいとき。

## hash
- e4de45ad5b70acb09726433704340627b3e9b239fac7df51f14f363006d79b8d
