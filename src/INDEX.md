...
## Summary
- `cmoc` の各サブコマンド実装を束ねるルーティング層で、個別コマンドの実行入口と、review・apply・session などの責務境界を読む起点になる。
- `doctor.py`、`indexing.py`、`eval_oracle.py`、`review_*` 系、`tui.py` は、それぞれ CLI 入口や実行フローの委譲先を分けて持ち、`apply` と `session` は配下パッケージとしてさらに個別処理を分担する。
- この階層は、どのサブコマンドがどの実装へ流れるかを最初に切り分けたいときに読む。一方で、共通 runtime や git 基盤、各サブコマンドの内部詳細だけが目的なら、ここではなく下位モジュールへ進む。

## Read this when
- `cmoc` のどのサブコマンドがどの実装ファイルへ対応するかを見分けたいとき。
- review・apply・session のような大きな責務の境界を確認してから、個別実装へ進みたいとき。
- サブコマンドの起動入口だけを確認したいときや、まず読むべき下位モジュールを絞り込みたいとき。

## Do not read this when
- 個別サブコマンドの引数、エラー処理、出力形式を直接追いたいとき。
- 共通 runtime、git 操作、worktree/state の基盤だけを調べたいとき。
- 特定機能の詳細が目的で、対応する下位モジュールがすでに分かっているとき。

## hash
- 1ad84a69e28cac203ff448316690ccc172f7a89b3c411cad84cd5eb64eef3603
