...
## Summary
- `src/sub_commands` 配下の各サブコマンド実装へ進むための入口。ここでは共通ルーティングの細部は追わず、個別サブコマンドの責務境界を見分けるために下位モジュールへ分岐する。
- この階層は `apply`、`doctor`、`eval_oracle`、`indexing`、`review`、`session`、`tui` の各実装へ案内する役割を持ち、サブコマンドごとの処理入口と関連補助モジュールを切り分けて読むときに使う。
- 各サブコマンドの引数定義、実行フロー、出力やエラー処理を確認したい場合は、この階層ではなく該当する個別実装へ進む。

## Read this when
- sub_commands 配下のどのサブコマンド実装に進むべきかを先に絞り込みたいとき。
- 個別サブコマンドの実行入口や責務の境界を確認したいとき。
- サブコマンド全体の構成を俯瞰して、目的の実装ファイルを見分けたいとき。

## Do not read this when
- 共通 runtime 操作や基盤処理だけを調べたいとき。
- 個別サブコマンドの引数、制御フロー、出力、エラー処理を直接追いたいときは、該当する下位実装を読む方がよい。
- この階層ではなく、特定のサブコマンドの本体や補助モジュールを読むべきことが明らかなとき。

## hash
- b8a54966f634c5d2df44bfe9cb40df5ce03eafc854b3145eb03e21e6fd4319a7
