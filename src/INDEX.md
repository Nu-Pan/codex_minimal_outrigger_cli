...
## Summary
- `src/sub_commands` は `cmoc` の各サブコマンド実装への入口を束ねる階層で、個別コマンドの本体へ進む前にこの配下がどの操作群を扱うかを確認したいときのルーティング先である。
- `apply` は apply 系サブコマンド群の実行入口をまとめる。`abandon` は active な apply run の破棄と後始末、`fork` は対象選定から適用・commit・state 更新・結果出力までの apply fork 全体、`fork_report` は fork 結果レポート生成、`join` は整合確認・差分判定・merge・cleanup を確認したいときに進む。
- この階層は、apply 系の各コマンドの役割だけを確認したいときに読む。一方で、具体的な引数定義や実行ロジック、session / review / tui など他系統の処理を知りたい場合は、対応する下位モジュールや別階層を読むべきである。

## Read this when
- apply 系サブコマンド群の実装パッケージの役割を確認したいとき。
- `cmoc apply abandon` の挙動、失敗条件、破棄後の cleanup を追いたいとき。
...
## Do not read this when
- 具体的な引数定義や個別の実行ロジックだけを知りたいときは、ここではなく各コマンド本体を読む。
- session の生成・join・report など、abandon 以外の apply フローを知りたいときは、対応する下位モジュールを読む。
- report だけを見たい場合に、fork 本体のループ制御や worktree 管理まで読む必要がないときは、`fork_report` に直接進む。
- join 以外の apply フローや branch/worktree の基礎操作だけを確認したいときは、`join` 以外の対象を読む。
- 正本仕様を探しているだけなら、実装ではなく oracle 側の文書を読むべきである。

## hash
- d5d945b21b3a2f505e227a4a0abae779c4f7deeea316520617b3799873c6eaad
