...
## Summary
- 対象ディレクトリは oracle 系サブコマンドをまとめる package の境界で、oracle サブコマンド群への入口です。
- 配下には編集、調査、レビューの実装と、レビューのパス・対象・ループ・レポート・INDEX 統合を担う補助実装があります。

## Read this when
- oracle 系サブコマンドの package 構成や入口を確認するとき。
- oracle サブコマンドの実行フロー、レビューのライフサイクル、対象範囲、結果統合またはレポート生成の実装を調査するとき。

## Do not read this when
- 個別の oracle サブコマンドやレビュー補助機能の詳細だけを確認したいときは、該当する下位実装を直接読む。
- oracle の調査・編集契約やプロンプト内容そのものを確認したいときは、対応する oracle 仕様を直接読む。

## hash
- 29eebbdf07d39bc42c7e9948aaa8e6cf1faa970d1942a17272ef9ec8e5489133
