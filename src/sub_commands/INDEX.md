...
## Summary
- oracle 系サブコマンドの実装をまとめるパッケージ。oracle の編集・調査・レビューを入口とし、レビュー対象列挙、パス解決、ループ処理、レポート生成、INDEX 更新などの下位実装へ案内する。

## Read this when
- oracle 系サブコマンドの構成や、編集・調査・レビュー処理の入口を確認するとき。
- oracle review の対象選択、パス処理、レビュー実行、レポート、INDEX 更新の実装先を探すとき。

## Do not read this when
- 個別サブコマンドの詳細な挙動だけを確認したい場合は、該当する実装ファイルを直接読む。
- 共通の CLI runtime や Codex TUI 自体の実装を調査するとき。

## hash
- 7d4d00383eca26ca4415f67eeba4c9934a9c3a5e3dba6635a01ca8a45c742071
