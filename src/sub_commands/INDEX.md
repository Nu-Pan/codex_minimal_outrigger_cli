...
## Summary
- oracle 系サブコマンドの実装をまとめるディレクトリ。oracle 編集・調査・レビューの各 CLI 入口と、レビュー対象列挙、finding 処理、パス解決、レポート生成、INDEX 差分反映などの下位実装への入口を提供する。

## Read this when
- oracle 系サブコマンドの実装構成や、各サブコマンド・レビュー関連処理の入口を確認するとき。
- oracle edit、oracle investigation、oracle review の実行フローや関連実装の所在を把握するとき。

## Do not read this when
- 特定のサブコマンドやレビュー処理の詳細を調査する場合は、対応する実装ファイルを直接読む。
- oracle サブコマンド共通の入力部品、TUI 起動パラメータ、Git 操作など単一責務の詳細だけを確認する場合は、該当する下位実装を直接読む。

## hash
- 39f8bc34f87636ec86a06ae2f4dd3d171ef226a65068757379f3bd1713c4a8c2
