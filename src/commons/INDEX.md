...
## Summary
- CLI サブコマンド共通の実行ライフサイクルを管理する実装。work root 検査、doctor 前処理、サブコマンドログ、feedback invocation、step 通知、完了・失敗処理、終了コード化、エラー表示、端末通知を一元化する。
- サブコマンド実行中の step 開始通知、ユーザー中断状態の記録、失敗時のログ終了処理、完了サマリー出力など、実行ランナーを補助する関数群を含む。CLI コマンドの個別処理内容ではなく、共通実行基盤を確認するための入口。

## Read this when
- CLI サブコマンドの実行順序、共通前処理、ログ記録、step 表示、完了・失敗時の挙動を変更または調査するとき。
- サブコマンドの終了コード、例外の表示経路、KeyboardInterrupt、feedback invocation、Windows terminal result 通知の連携を確認するとき。
- work root と repository root の使い分けや、共通ランナー外からの中断状態管理を確認するとき。

## Do not read this when
- 特定サブコマンドの個別ビジネスロジック、引数定義、またはコマンド固有の事前条件だけを調べるときは、そのサブコマンド実装を直接読む。
- ログの保存形式、エラー文面、doctor 前処理、feedback 保存、端末通知の詳細仕様を確認するときは、対応する runtime モジュールや正本仕様を直接読む。
- CLI 以外の処理や、共通ランナーを利用しない内部機能だけを調べるとき。

## hash
- 84351276fdb4f19f0eeb1aa453c6944079d26e7166b3e4e4305794df7e58c6b1
...
## Summary
- Git repository と filesystem の共通境界として、Git コマンド、branch、linked worktree、worktree snapshot の操作を扱う。
- Git ignore の検証・保証と、oracle／realization file の列挙・分類を提供する。
- repository path、symlink、特殊 file、managed worktree の安全性を検査する runtime helper 群であり、これらの挙動や境界を変更・確認するときの入口となる。

## Read this when
- Git コマンド実行、branch・worktree 管理、worktree snapshot の取得・復元を調べるとき。
- Git ignore や .cmoc/gu の追跡状態を検証・更新するとき。
- oracle／realization file の列挙または path 分類を調べるとき。
- managed path、symlink、Git metadata、特殊 file の安全性検証を扱うとき。

## Do not read this when
- Git 境界、worktree、ignore、oracle／realization file 分類を扱わず、別の runtime helper の責務だけを調べるとき。
- 正本仕様の意図や制約を確認するときは、この実装ではなく対応する oracle 文書・仕様を直接読むとき。

## hash
- 8ec22af9cfbe3044bdc5c4d7e49854fcc6bcebc59e9cda1f40efe798ba03fc66
