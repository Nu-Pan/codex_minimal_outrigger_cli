...
## Summary
- INDEX.md の検査・生成・更新・commit を一貫して担う indexing lifecycle の共通実装。対象ディレクトリの走査、既存 entry の hash による再利用、Codex による不足 entry の生成、Structured Output の検証、INDEX.md の書き込み、更新差分の commit、排他 lock を扱う。

## Read this when
- INDEX.md の自動生成・鮮度判定・entry 再利用・hash 検証を変更または調査するとき
- indexing preflight、並列 entry 生成、Codex 実行コンテキスト、linked worktree 間の lock を変更または調査するとき
- INDEX.md の書き込み、symlink・binary・git ignore・memo の除外、commit 処理を変更または調査するとき

## Do not read this when
- 単一の INDEX.md entry の内容や生成用 Structured Output schema だけを確認したいときは、index entry の oracle 定義または直接の生成処理を読む
- INDEX.md と無関係な CLI 実装、一般的な git 操作、Codex preflight 以外の runtime 機能を変更または調査するときは、対応する専用モジュールを読む

## hash
- 533c696aa274c1d3abb88b101060923ac759bef9852ce5bdd9854e49a8230885
...
## Summary
- CLI サブコマンドの共通実行ライフサイクルを管理するモジュール。work root 検査、doctor preprocess、サブコマンドログ、step 通知、完了サマリー、戻り値・例外の共通処理を提供する。
- サブコマンド実装の実行経路や、ログ・コンソール出力・エラー終了・step 管理の挙動を確認するための入口。

## Read this when
- CLI サブコマンドの共通ラッパーや実行順序を変更するとき
- サブコマンドログ、step 通知、完了サマリーの出力を調査するとき
- サブコマンドの戻り値や例外がどの終了コード・出力先になるか確認するとき
- work root 検査や doctor preprocess の呼び出し位置を確認するとき

## Do not read this when
- 特定サブコマンド固有の処理や引数定義だけを変更・調査するときは、そのサブコマンドの実装を直接読む
- ログの保存形式やエラー表示文面の正本仕様を確認するときは、対応する oracle 文書を先に読む

## hash
- 81dbd2341537a730f373a91002f5e27be0fa190230f5b76d4e662c11c13962d8
...
## Summary
- editing run の worktree 解決、プロセス追跡、ライフサイクル直列化、停止・cleanup を担う共通ランタイムモジュール。branch から worktree を求める処理と、run 本体および Codex child process group の同一性確認付き停止処理への入口。

## Read this when
- editing run の worktree 解決や branch と worktree の対応を確認・変更するとき
- run process tracking file、ライフサイクル lock、process identity の保存・読取・削除を扱うとき
- run abandon、error cleanup、追跡中 Codex subprocess の安全な停止処理を確認するとき

## Do not read this when
- editing run の仕様そのものを確認する場合は、対応する oracle 文書を先に読むとき
- 一般的な git 操作、path 定義、エラー型の詳細だけを扱う場合は、各専用 runtime module を直接読むとき
- Codex subprocess の起動や tracking 情報の登録側だけを変更する場合は、呼び出し元の run 実装を直接確認するとき

## hash
- 687285ac6c553f836600335181c6fc0e20a8ea8410fd6649c13191c1639f3587

# `runtime_run_lifecycle.py`

## Summary
- editing run の開始、active run の解決、state 遷移、work unit の rollback・commit、INDEX 更新、差分分類、想定外 path 検査、oracle 差分取得、run worktree の cleanup 判定を担う共通 lifecycle 実装。EditingRunContext と lifecycle lock を共有する処理群の入口。

## Read this when
- editing run の開始・joinable/error 遷移・worktree 管理を変更または調査するとき
- run branch や session branch の差分許可範囲、rename を含む変更 path 分類を確認するとき
- run worktree での commit、rollback、INDEX 更新、oracle 差分取得の挙動を確認するとき

## Do not read this when
- 個別の session state schema や state file の永続化仕様だけを確認するとき
- INDEX 生成処理そのものだけを変更または調査するとき
- CLI サブコマンド固有の workload 実装や realization file の内容を確認するとき

## hash
- 697a28bea179548216c9926e73ec2ca4831a11332a72e058fa10660555eb9573
