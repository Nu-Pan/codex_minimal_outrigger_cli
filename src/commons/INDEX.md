...
## Summary
- CLI サブコマンドの共通実行ライフサイクルをまとめる実装。work root 実行前提の検査、サブコマンドログの開始・完了イベント、標準出力への進捗・完了サマリー、戻り値の終了コード化、例外のエラー表示を一箇所で扱う。
- 通常の runtime state は repo root、初期化対象を扱う場合だけ work root を使い、サブコマンドログは repo root に置くという実行時 root の使い分けもここで決める。

## Read this when
- CLI サブコマンド全体に共通する開始表示、完了表示、終了コード、例外時の表示、サブコマンドログ記録の挙動を確認・変更したいとき。
- サブコマンド実装を共通ラッパーから呼び出す方法、事前検査の差し込み、コマンド名や argv のログ記録、runtime root の選択を確認したいとき。
- cmoc が work root 以外で実行された場合のエラー条件や、完了時 stdout サマリーの項目を確認したいとき。

## Do not read this when
- 個別サブコマンドの業務処理そのもの、引数定義、Typer app への登録箇所を調べたいだけのとき。
- ログファイルの具体的な保存形式、現在のサブコマンド logger の保持機構、イベント書き込み実装を調べたいとき。
- repo root、work root、時刻文字列、経過時間フォーマットの算出方法を調べたいとき。
- cmoc 独自例外の構造やエラーメッセージ描画の詳細を調べたいとき。

## hash
- d5e111ec2f65352f79b60c575c18d47f7c7b7f002d9041724c89bb01009858af
