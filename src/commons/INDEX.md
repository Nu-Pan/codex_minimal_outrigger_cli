...
## Summary
- CLI サブコマンドの共通実行ライフサイクルを扱う実装。work root 実行の検査、runtime state の置き場所選択、サブコマンド logger の設定、開始・実行・完了の標準出力、戻り値の終了コード化、例外時のエラー表示と終了コード化を一括して管理する。
- サブコマンド完了時の標準サマリー出力と、cmoc が work root で実行されていることを検査する補助処理も含む。

## Read this when
- CLI サブコマンド実装を共通ラッパー経由で実行する流れ、開始・完了表示、サブコマンドログ生成、終了コード処理、例外表示の挙動を確認または変更したいとき。
- init のように runtime state を work root 側へ置く必要があるサブコマンドと、通常の repo root 側へ置くサブコマンドの分岐を確認したいとき。
- カレントディレクトリが work root であることを要求する実行前検査や、その失敗時の利用者向けエラー内容を確認したいとき。
- サブコマンドごとの logger を現在の実行コンテキストへ設定・解除する処理や、quota wait・elapsed・returncode をログと stdout に反映する箇所を追いたいとき。

## Do not read this when
- 個別サブコマンドの業務処理、引数定義、Typer command 登録そのものを調べたいだけのとき。該当するサブコマンド実装を直接読む方がよい。
- ログイベントの保存形式、現在のサブコマンド logger の内部実装、ログファイルの構造を調べたいとき。ログ管理側の実装を直接読む方がよい。
- repo root・work root の検出規則、時刻表示、duration 表示の詳細を調べたいとき。path や表示用 helper の実装を直接読む方がよい。
- CmocError の構造やエラーメッセージ整形全般を変更したいとき。エラー処理側の実装を直接読む方がよい。

## hash
- 54cb8d73a73d1e47b127c23b53ca34e980234df7bde72d6fd36574612de1f5cf
