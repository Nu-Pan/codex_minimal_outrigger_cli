...
## Summary
- リポジトリ・worktree・cmoc root の解決、実行時刻や経過時間の整形、timestamp 付き path の排他的予約、各種 runtime ディレクトリ／設定 path の構築、cwd の一時切替を担う共通 runtime path utility。root 解決や pushd、保存先 path の挙動を確認する際の入口。

## Read this when
- root placeholder の実パス解決や、cwd を起点にした repository/worktree 探索を変更・調査するとき
- session、report、log、schema、worktree など cmoc の runtime 保存先を扱う処理を変更・調査するとき
- timestamp、console timestamp、duration 表示、timestamp path の衝突回避を確認するとき
- process-global な cwd 切替や pushd の thread/context 制御を変更・調査するとき

## Do not read this when
- 特定サブコマンドの業務ロジックや report／log の内容形式だけを確認したいときは、該当するサブコマンド実装や正本仕様を直接読む
- root placeholder の定義や一般的な path 解決規則そのものを確認したいときは、`basic.path_model` と対応する oracle file を直接読む

## hash
- 0bfa25c7082f4af28c66b776bf22abb1f2fec496fdc2b9707d6f6f17cb6125de
...
## Summary
- editing run の worktree 解決、run lifecycle lock、process tracking、親 run process と Codex child process group の安全な停止・cleanup を担う共通 runtime 境界。join/abandon や error cleanup で、同一 session の状態・worktree identity・fail-closed な process 同一性検証を確認する入口になる。

## Read this when
- editing run の worktree lookup、join/abandon、error cleanup、process tracking、run process または Codex child process の停止処理を変更・調査するとき。
- run state の lock、tracking file の検証・削除、PID 再利用対策、process group の停止完了確認を確認するとき。

## Do not read this when
- worktree 解決や run process の lifecycle に関係せず、個別の git 操作、path 定義、Codex profile の低レベル process helper だけを変更・調査するとき。
- editing run の正本仕様を確認する必要があるときは、先に対応する oracle 文書を読む。

## hash
- 840f4b5af73919afcb56b06ce7ca04501103e94c362f0d108529ff4b537e8270
