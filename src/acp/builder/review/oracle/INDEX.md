...
## Summary
- レビューで得た所見リストに対し、重複・矛盾を整理する編集操作を返すための構造を定義する。削除、単一所見の置換、複数所見の統合を表現し、整理後に残す所見では重大度、見出し、根拠となる正本仕様断片、理由を保持する。

## Read this when
- 複数のレビュー所見を、削除・置換・統合の操作として機械的に整理する出力契約を確認したいとき。
- 同じ問題を指す所見や互いに矛盾する所見を、最終的な所見リストへどう反映するかを表すデータ構造を扱うとき。
- レビュー所見の重大度、根拠となる正本仕様断片、整理理由を含む編集結果の形式を実装・検証するとき。

## Do not read this when
- 個々の正本仕様断片そのものの妥当性や内容を確認したいとき。
- レビュー所見を発見する判定基準や検出ロジックを知りたいだけで、所見リストの整理操作の出力契約を扱わないとき。
- ルーティング文書、実装品質基準、または一般的なファイル分類の説明を確認したいだけのとき。

## hash
- c192df5d7964f5e2aa570d02157a0c4c2b39e2b504dddc5079835dd5e704ebfc
