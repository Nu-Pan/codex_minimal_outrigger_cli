...
## Summary
- この ACP builder 領域には indexing 専用の実装がありません。文書検索索引の明示同期は CLI と共通検索基盤が担います。

## Read this when
- ACP builder に indexing 専用処理があるか、またはそこを経由する経路があるかを確認するとき。

## Do not read this when
- 文書検索索引の同期動作を調べたり変更したりするときは、同期を担う CLI と共通検索基盤へ直接進んでください。
