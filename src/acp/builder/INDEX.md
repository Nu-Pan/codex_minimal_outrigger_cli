...
## Summary
- 正本仕様断片レビュー工程で使う AI 呼び出しパラメータ構築と Structured Output 契約をまとめる領域。レビュー対象と関連する正本仕様断片からの新規所見列挙、所見リストの重複・矛盾整理、所見を支持または反証する理由の追加収集、所見を人間へ提示するかの採否判定を扱う。
- 各工程は、正本仕様断片を読み取り専用の根拠として扱う前提、既知情報との重複回避、推測ではなく具体的な正本仕様断片に基づく判断、機械処理可能な結果形式への接続を確認する入口になる。

## Read this when
- 正本仕様断片レビューで使う AI 呼び出しについて、prompt、補助文脈、モデル区分、推論量、ファイルアクセス方針、出力契約との接続を確認または変更したいとき。
- レビュー対象の正本仕様断片と関連する正本仕様断片から、既知所見と重複しない新規所見を列挙する処理や、新規所見がない場合の扱いを確認したいとき。
- 収集済み所見リストの内容的な重複や相互矛盾を、削除・置換・統合として整理する AI 出力契約や、その整理用 prompt を確認したいとき。
- 対象所見が妥当である理由、または妥当ではない理由を、既知理由と重複しない正本仕様断片由来の根拠として追加調査する処理を確認したいとき。
- 所見本文、支持理由、反証理由を AI 呼び出しへ渡し、人間へ提示すべきかを理由付きで判定する構成を確認したいとき。

## Do not read this when
- 個別の正本仕様断片本文をレビューして、具体的な仕様矛盾、表記揺れ、誤字脱字などの所見そのものを探したいとき。この領域はレビュー工程の呼び出し構築と出力契約を扱い、根拠本文は対象の正本仕様断片を読む。
- 正本仕様断片と実現ファイルの基本定義、責務境界、正本仕様断片としての一般基準を確認したいとき。基本概念や標準を定義する文書へ進む。
- 正本仕様断片レビュー全体の CLI 引数解析、サブコマンド配線、実行順序、所見の保存・表示・適用処理を確認したいとき。呼び出し側の command 実装や所見管理側を読む。
...
## hash
- faec44a59cb3164266bbeb325a22a001c8b8c1c778fb95dfb7fc48f8c70a6a53
