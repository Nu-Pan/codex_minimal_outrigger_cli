...
## Summary
- AI エージェントへ提示するファイル読み書き規則のプロンプト断片を、指定された読み書きモードから構築する実装。
- 作業ルート、oracle、memo に対する読み取り・書き込み禁止範囲をモード別に本文化し、構造化文書として返す。
- 対応する oracle 実装断片を根拠として持つ realization implementation であり、ファイルアクセス制約プロンプトの生成ロジックを確認する入口となる。

## Read this when
- ファイルアクセス制約を AI エージェント向けプロンプトへどう表現しているか確認したいとき。
- 読み書きモードごとの作業ルート、oracle、memo の許可・禁止範囲を生成する実装を変更するとき。
- ファイルアクセスモードの追加・変更に伴い、プロンプト本文や不正モード時の扱いを確認するとき。

## Do not read this when
- ルーティング文書や INDEX.md の読み進め規則そのものを確認したいだけのとき。
- パスキーワードや作業ルート解決の定義を確認したいときは、パスモデル側を直接読む。
- ファイルアクセスモード列挙値そのものの定義や型を確認したいときは、基本 ACP 定義側を直接読む。
