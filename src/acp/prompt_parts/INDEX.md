...
## Summary
- AI エージェントに提示するファイル読み書き規則のプロンプト断片を、指定されたアクセスモードから構築する実装。作業対象ルートを解決し、読み取り専用、oracle 限定読み取り、実装書き込み、oracle 書き込み、リポジトリ書き込みの各プリセットに応じて、禁止される読み書き範囲を本文化して構造化文書として返す。

## Read this when
- AI エージェントへ渡すファイルアクセス制約の文面、見出し、モード別の禁止範囲を確認または変更したいとき。
- 読み書きモードプリセットと、作業対象ルート、oracle、.agents、memo に対するアクセス禁止ルールの対応を確認したいとき。
- ファイルアクセス規則を生成するプロンプト部品が、どの構造化文書タイトルと本文を返すかを追う必要があるとき。

## Do not read this when
- ファイルアクセス規則ではなく、ルーティング規則、実装標準、oracle 標準など別種のプロンプト部品を確認したいとき。
- アクセスモードの列挙値そのものの定義や意味を確認したいだけのときは、その型を定義している基礎モジュールを読む。
- 作業対象ルートの算出方法やパスキーワードの定義を確認したいだけのときは、パスモデル側の実装または仕様を読む。
