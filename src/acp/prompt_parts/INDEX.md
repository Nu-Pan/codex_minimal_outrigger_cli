...
## Summary
- AI エージェントに提示するファイル読み書き制約の本文を、指定された読み書きモードから組み立てるプロンプト部品。
- 作業ルート、oracle、memo に対する読み書き可否の組み合わせをモード別に切り替え、構造化文書として返す。

## Read this when
- エージェントに渡すファイルアクセス規則の文面を確認・変更する必要があるとき。
- 読み取り専用、oracle 限定読み取り、実装編集、oracle 編集、リポジトリ編集などのモードごとの許可範囲を確認するとき。
- 作業ルートや memo、oracle に対する禁止事項がプロンプトへどう反映されるかを追うとき。

## Do not read this when
- ルーティング規則、タスク本文、実装標準など、ファイルアクセス以外のプロンプト部品を確認したいとき。
- 作業ルートの解決方法そのものやパスキーワードの定義を確認したいとき。
- 読み書きモードの列挙値や型定義だけを確認したいとき。
