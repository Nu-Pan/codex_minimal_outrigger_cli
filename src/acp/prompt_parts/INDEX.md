...
## Summary
- エージェント呼び出しへ渡す完全なプロンプト構造を組み立てる実装。役割・概要・ゴール・ファイルアクセス規則・ルーティング規則・補助プロンプトを基礎として並べ、指定された標準プロンプト群を依存関係込みで追加する。
- プロンプト内の cmoc 固有語やルートトークンを、Codex CLI 向けの表現や実際の作業ルートに置換する処理も担う。

## Read this when
- エージェントに渡す最終的なプロンプト構成、標準プロンプトの注入順序、または標準間の依存関係を確認・変更したいとき。
- Codex CLI 向けに、仕様・実装・レビュー・INDEX.md エントリー関連の説明文がどのように言い換えられるかを確認したいとき。
- ルートトークンがプロンプト生成時にどの実パスへ解決されるか、または解決失敗時の扱いを確認したいとき。

## Do not read this when
- 個別の標準プロンプト本文そのものを変更したいとき。この対象は各標準ビルダーを呼び出す側であり、本文定義はそれぞれの標準用モジュールを読む。
- ファイルアクセス規則やルーティング規則の内容を確認・変更したいとき。この対象はそれらを組み込むだけなので、各規則のビルダー定義を直接読む。
- 生成された構造化ドキュメントの表現形式やデータ構造を調べたいとき。この対象ではなく、構造化ドキュメントの型定義を読む。

## hash
- c7edde2361e9910e36ec7fd228cb51b8f99a2562ec7efd7f30f5eb65d29e2d21
