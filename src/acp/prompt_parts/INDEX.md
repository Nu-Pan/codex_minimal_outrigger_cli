...
## Summary
- AI エージェントに提示するファイル読み書き規則の文書断片を、読み書きモードプリセットごとに構築する実装。
- 作業ルートを解決し、読み取り専用、oracle 専用読み取り、realization 編集、oracle 編集、repo 編集の各モードに応じて、禁止対象となるツリーや memo の扱いを箇条書き本文として返す。
- 戻り値は規則名と本文を持つ構造化文書であり、実際のファイルアクセス制御ではなく、ACP 向けプロンプト本文の生成を担う。

## Read this when
- ファイル読み書き規則として AI に提示される文言を確認または変更したいとき。
- 読み書きモードプリセットごとの work root、oracle、memo に対する許可・禁止の境界を確認したいとき。
- ACP のプロンプト部品として、ファイルアクセス制約の構造化文書がどのように組み立てられるかを追いたいとき。
- 新しい読み書きモードを追加した結果、このプロンプト部品の分岐や未対応モードの扱いを確認する必要があるとき。

## Do not read this when
- 実際のファイルシステムアクセスを強制する処理やサンドボックス実装を探しているとき。
- 作業ルートの解決規則そのものを確認したいとき。
- 読み書きモードプリセットの列挙値や型定義そのものを確認したいとき。
- ファイルアクセス規則以外の ACP プロンプト部品を変更したいとき。

## hash
- 22dec9a526ae87044a6c980da16c35587b1762da38567222d88d364f5fd67f5e
