...
## Summary
- agent call に渡す完全なプロンプトを StructDoc のリストとして組み立てる実装。role・summary・goal・ファイルアクセス規則・ルーティング規則・補助プロンプトを基礎に、指定された標準プロンプト群を依存関係込みで追加する。
- Codex CLI 向けに、仕様・実装・ブランチ・ルートトークンなどの用語を作業者向け表現や実パスへ置換する前処理も担う。

## Read this when
- agent に渡す最終プロンプト全体の構成順、含める基本セクション、補助プロンプトの扱いを確認したいとき。
- oracle、realization、レビュー基準、INDEX.md エントリー基準などの標準プロンプトを、どの真偽値指定で注入するかを確認したいとき。
- 標準プロンプト同士の依存関係、たとえば INDEX.md エントリー基準を入れる際に仕様・実装の基本情報や各標準も同時に入る挙動を調べたいとき。
- agent 向けプロンプト内の用語やルートトークンが、Codex CLI 実行時にどのような表現へ書き換えられるかを確認したいとき。

## Do not read this when
- 個別のファイルアクセス規則、ルーティング規則、各標準プロンプトの本文そのものを確認したいだけのときは、それぞれの builder 実装を直接読む。
- StructDoc や StructCodeBlock のデータ構造、Markdown などへの変換処理を調べたいときは、構造化ドキュメント側の実装を読む。
- cmoc ルート、リポジトリルート、作業ルートなどのパス解決仕様を詳しく確認したいときは、パスモデル側の実装を読む。
- 生成されたプロンプトを実際に agent call へ渡す呼び出し側の制御や CLI サブコマンドの流れを調べたいときは、この組み立て関数ではなく上位の呼び出し元を読む。

## hash
- a04ca8549d6a0e8c6769a88480319b6784e57d27a5cef809c8cb34eb61b63f95
