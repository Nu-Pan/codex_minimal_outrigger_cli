...
## Summary
- agent 呼び出しに渡す完全なプロンプトを、基本プロンプト、ファイルアクセス規則、ルーティング規則、任意追加プロンプト、必要に応じた各種基準プロンプトから組み立てる実装。
- root token を実パスへ解決し、Codex CLI 向けに用語・タイトル・コードブロック内のパス表記を置換する補助処理も担う。
- 各種基準プロンプトの有効化フラグ間の依存関係を解決し、要求された基準に必要な前提基準を自動的に含める入口になっている。

## Read this when
- agent 向けの完全なプロンプトがどの順序・構成で生成されるかを確認したいとき。
- ファイルアクセス規則、ルーティング規則、補助プロンプト、仕様・実装・レビュー・INDEX.md 関連基準の注入条件を変更したいとき。
- Codex CLI 向けに用語や root token がどのように実パス・日本語表現へ置換されるかを調べたいとき。
- 基準プロンプトのフラグを追加・削除・改名し、それに伴う依存関係や注入順序を調整したいとき。

## Do not read this when
- 個別のファイルアクセス規則、ルーティング規則、仕様文書基準、実装保守基準など、それぞれの基準本文だけを確認したいときは、対応する基準構築処理を直接読む。
- agent に渡された構造化ドキュメントの表現そのものやレンダリング方法を調べたいときは、構造化ドキュメントを定義する処理を読む。
- root token や各 root path の定義・解決規則そのものを確認したいときは、パスモデルの実装を読む。
- 実際の CLI コマンド実行、サブコマンド制御、作業ディレクトリ管理の挙動を調べたいときは、このプロンプト組み立て処理ではなく呼び出し側の実装を読む。

## hash
- 7c0c82daa24ccfc221c200520a3eff065400a0b4a6a1ca11677d2ad6a22154e1
