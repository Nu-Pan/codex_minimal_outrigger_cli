...
## Summary
- agent call に渡す完全なプロンプトを、役割・概要・完了条件・ファイルアクセス規則・ルーティング規則・追加プロンプト・任意の標準プロンプト群から組み立てる実装。
- 各標準プロンプトの有効化フラグ間の依存関係を解決し、必要な基本情報や標準文書を不足なく追加する入口になっている。

## Read this when
- agent に渡す最終的なプロンプト構成、標準プロンプトの注入順序、またはフラグ指定による依存プロンプトの自動追加を確認・変更したいとき。
- oracle standard、realization standard、review/apply review standard、index entry standard などを有効化した際に、どの補助プロンプトが一緒に含まれるかを調べたいとき。
- role、summary、goal、file access rule、routing rule、aux prompt を含む基本プロンプトの組み立て箇所を探しているとき。

## Do not read this when
- 個々の標準プロンプト本文の内容や文言を確認・変更したいだけのときは、それぞれの標準プロンプトを構築する別対象を読む。
- ファイルアクセス規則やルーティング規則そのものの詳細を確認・変更したいだけのときは、それらを構築する別対象を読む。
- agent 呼び出し処理、プロンプト送信処理、または StructDoc 自体のデータ構造を調べたいときは、この対象ではなく呼び出し側や基盤型の実装を読む。
...
## Summary
- AI エージェント向けプロンプトのうち、ファイル読み書き規則セクションを組み立てる実装。読み書きモードプリセットに応じて、作業ルート外、oracle、memo、作業ルート内の読み書き禁止範囲を本文化し、見出し付きの構造化文書として返す。
- ファイルアクセス制限の文言を生成する入口であり、各モードがどの領域を読み書き禁止にするかを確認するための対象。

## Read this when
- AI エージェントへ渡すファイル読み書き規則プロンプトの生成内容を確認・変更したいとき。
- 読み取り専用、oracle 読み取り、実装書き込み、oracle 書き込み、リポジトリ書き込みの各モードで、どのパス範囲が禁止されるかを確認したいとき。
- ファイルアクセスモードの値から構造化文書の見出しと本文がどのように作られるかを追いたいとき。
- 作業ルート、oracle、memo に関するアクセス制限文言の出力差分を調整したいとき。

## Do not read this when
- ファイルアクセスモードそのものの列挙定義や値を確認したいだけのときは、モード定義側を読む。
- 作業ルートの解決方法やパス概念の定義を確認したいときは、パスモデル側を読む。
- 構造化文書のデータ構造や文字列整形ヘルパーの仕様を確認したいときは、それらの基盤実装を読む。
- 実際のファイルシステム権限 enforcement やサンドボックス制御の実装を探しているときは、このプロンプト生成処理ではなく実行制御側を読む。
