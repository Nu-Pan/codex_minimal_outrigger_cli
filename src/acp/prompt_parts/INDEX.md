...
## Summary
- AI エージェントへ提示するファイル読み書き規則のプロンプト断片を、指定されたアクセスモードに応じて構築する実装。作業ルート外、oracle、memo、リポジトリ全体などに対する読み書き禁止条件をモード別に本文化し、構造化文書として返す。

## Read this when
- エージェントに渡すファイルアクセス制約の文面が、各読み書きモードでどのように生成されるか確認したいとき。
- 読み取り専用、oracle 専用読み取り、実装編集、oracle 編集、リポジトリ編集の各モードに対応する禁止範囲を変更・検証したいとき。
- 作業ルート、oracle、memo に対するアクセス禁止ルールをプロンプト部品として組み立てる処理を追いたいとき。

## Do not read this when
- ファイルアクセスモードそのものの列挙値や意味を確認したいだけなら、モード定義を直接読む。
- 作業ルート解決の仕組みを確認したいだけなら、パスモデル側の実装を直接読む。
- 生成済みプロンプト全体の構成や他のプロンプト部品との並びを確認したい場合は、プロンプト組み立て側の呼び出し元を読む。
