...
def build_review_oracle_standard() -> StructDoc:
    """
    oracle file のレビューで所見を列挙する作業の規範文章を構築する
    """
    standards = [
        Standard(
            ...
            examples=[
                "同じ状態の結合処理について、一方が必ず続行、他方が必ず中止と定めており例外関係も読めないなら fatal 所見にする",
        ...
        Standard(
            ...
            backgrounds=[
                ...
                "oracle file のレビューでは、リポジトリ固有の事情に依存しない汎用的な観点で oracle file を評価する",
            ],
            requirements=[
                Requirement(
                    "禁止",
