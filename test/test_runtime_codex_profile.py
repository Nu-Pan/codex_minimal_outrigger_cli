...
def test_codex_overrides_generates_rooted_sandbox(tmp_path: Path) -> None:
    ...
    build_codex_override_args(
        ...
        CmocConfig(),
        root,
        [root / ".cmoc" / "gu" / "ar" / "log" / "tui" / "20260101_cmpl.md"],
    )
    ...
    with pytest.raises(CmocError, match="許可領域外"):
        build_codex_override_args(
            ...
            CmocConfig(),
            root,
            [root / ".cmoc" / "gu" / "ar" / "log" / "tui" / "20260101_orig.md"],
        )
...
def test_codex_overrides_allows_repo_agent_read_dirs_from_linked_worktree(
    tmp_path: Path,
) -> None:
    """linked worktree 実行時だけ repo 側 `.cmoc/g*/ar` を追加許可する。"""
    root = make_repo(tmp_path)
    linked = root / ".cmoc" / "gu" / "worktree" / "linked-read"
