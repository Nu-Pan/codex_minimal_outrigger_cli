...
def test_run_codex_exec_uses_stdin_and_writes_logs(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    (fake_cmoc_root / "bin" / "cmoc").write_text("#!/bin/sh\n")
    monkeypatch.setattr(cmoc_runtime, "cmoc_root", lambda: fake_cmoc_root)
    ...
    fake_codex.write_text(
        "\n".join(
            [
                ...
                "record.write_text(json.dumps({'args': args, 'stdin': stdin}))",
...
def test_run_codex_exec_polls_and_resumes_after_quota(tmp_path: Path, monkeypatch) -> None:
    ...
    argv_calls = [json.loads(line) for line in calls.read_text().splitlines()]
