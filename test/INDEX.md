...
## Summary
- oracle review ACP builder の parameter builder 群について、公開 API、モデル・推論設定、file access mode、schema、placeholder 補正、互換 builder の同値性を検証する realization test。oracle 側 schema との一致や動的入力・symlink path の保持も確認する。

## Read this when
- oracle/acp_builder/oracle/review の parameter builder、schema、adapter 公開面を変更・レビューするとき
- review 用 builder の prompt placeholder 補正、schema 出力、互換性、symlink 対応を検証するとき

## Do not read this when
- review ACP builder 以外の実装や、oracle schema 自体の仕様を直接確認したいとき
- 一般的な ACP builder の共通仕様だけを確認する場合

## hash
- 7f2b33b0e665783b3ada5dcd8a5702b53b1ab0e0f020365ddd628ee04659217b
...
## Summary
- oracle review の worktree 分離、snapshot commit からの fork、未コミット差分の拒否、INDEX.md のみの統合、競合解決、不要な差分の拒否を検証するテスト群。oracle review の run isolation・branch model・indexing 仕様に対応する挙動テストへの入口。

## Read this when
- oracle review の worktree 作成元、session branch、snapshot commit、review worktree の後処理を変更・調査するとき
- oracle review による INDEX.md の統合、preflight 生成物、merge conflict 解決を変更・調査するとき
- review worktree が INDEX.md 以外の差分を拒否する条件や、未コミット変更の扱いを確認するとき

## Do not read this when
- oracle review の finding 判定ロジックや Codex 出力 schema 自体を変更・調査するときは、対応する oracle review 実装・schema テストを直接読む
- 通常の INDEX.md 生成処理だけを変更・調査するときは、indexing 実装・テストを直接読む
- session fork や branch model 全般を変更・調査し、oracle review 固有の worktree 統合挙動を確認する必要がないとき

## hash
- 645648f5edfeeb1524865a0ca87665d1ed4a66584ffb87faf6eb3a8a299a0ac9
...
## Summary
- CLI の duration 表示、サブコマンドログ、並列イベント記録、error report、引数解析、scope 制約、work root 検証、completion probe、preflight、gitignore 更新を外部挙動として検証するテスト。関連する CLI 境界や runtime logging の変更時に、実装が仕様どおりか確認するための入口。

## Read this when
- CLI の error 出力形式や終了コードを変更するとき
- サブコマンドログ、preflight、completion、work root 制約の挙動を変更するとき
- scope option、gitignore 更新、duration 表示、並列 logger 記録を変更または検証するとき

## Do not read this when
- CLI の内部実装を調査するだけで、error・log・preflight・completion の外部挙動を変更しないとき
- oracle の仕様文書や個別サブコマンドの実装を直接確認することが目的のとき
- runtime CLI と無関係な機能のテストや実装を変更するとき

## hash
- c22655a2d709491097f5803723c0b5e735681c71ccf63be64cac38b72d2abf9c
