...
## Summary
- Codex subprocess の process group 追跡・停止・reap と、tracking file の更新および継承環境の扱いを検証する pytest。SIGTERM の遅延、leader 終了後の descendant 追跡、tracking 失敗時の cleanup、pidfd による安全な group 停止、壊れた stale tracking の拒否を対象とする。関連する runtime 実装の変更や subprocess cleanup の挙動を確認する際のテスト入口。

## Read this when
- Codex subprocess の起動、process group、pidfd、SIGTERM、tracking file、KeyboardInterrupt 時の cleanup を変更・調査するとき
- `commons.runtime_codex_profile` または `commons.runtime_run` の child process 停止・追跡ロジックを検証するとき
- stale process identity や process group の再利用に対する fail-closed 挙動を確認するとき

## Do not read this when
- Codex subprocess の追跡や process group cleanup に関係しない CLI 機能・テストを変更するとき
- 通常の Codex 実行結果や、tracking file を使わない runtime 機能だけを調査するとき
- 実装の詳細ではなく、編集実行時の process cleanup 仕様そのものを確認したいときは、先に対応する oracle 文書を読む

## hash
- 2fa678fbc765620c7763f2e93b712c2a40fd2369a4b08e8cc0b05d28960df4a3
...
## Summary
- workload fork と共通 editing run lifecycle の統合テストを扱う。realization apply/refactor の fork、run join/abandon、session state・worktree・branch・process tracking・report・INDEX 更新、失敗時 rollback と cleanup を検証する。

## Read this when
- realization apply/refactor fork の run lifecycle 挙動を変更・調査するとき
- run join または abandon の state 遷移、merge、cleanup、force-resolve を変更・調査するとき
- Codex child tracking、process 停止、fork report・lifecycle report、INDEX 更新制約を変更・調査するとき
- 中断・初期化失敗・commit 失敗・競合など editing run の異常系を検証するとき

## Do not read this when
- editing run の lifecycle や fork/join/abandon に関係しない単体処理を変更・調査するとき
- Codex CLI 出力自体の品質や、個別 helper の実装詳細だけを検証するとき

## hash
- 5d2cce86701c5556d1b7b8b6304553593e2e9e1d6ee489efe950f079c3a984b0
...
## Summary
- oracle review の review worktree lifecycle 全体を検証する回帰テスト。linked worktree と snapshot commit からの fork、run target 衝突、作成中断・例外時の cleanup、未コミット差分の拒否、active editing run 中の実行を扱う。
- review worktree で生成・preflight commit された INDEX.md の統合、INDEX.md 以外の差分拒否、merge conflict 解決、Git が quote するパスや未追跡 nested INDEX.md の処理を検証する。
- worktree・branch cleanup の失敗や dangling symlink をエラーとして報告する安全性も対象とする。review worktree lifecycle と INDEX 差分の外部契約をまとめて検証する入口。

## Read this when
- oracle review サブコマンドの worktree、branch、snapshot、run lifecycle、割り込み時 cleanup を変更・調査するとき
- oracle review が INDEX.md のみを session に統合する処理、preflight indexing、merge conflict 解決を変更・調査するとき
- review 実行後の隔離リソース削除失敗や不正な差分の扱いを確認するとき

## Do not read this when
- oracle review の所見列挙・challenge・advocate・judge の個別ロジックだけを変更・調査するとき
- 一般的な INDEX.md 生成処理や通常の run lifecycle を、oracle review との統合挙動を確認せずに変更・調査するとき
- 対象外のサブコマンドの worktree cleanup や branch lifecycle を扱うとき

## hash
- 88b67cf7c27c65198ee17fd8aa6a1e060d21857333ca653235104f5de4b2a5ee
