...
## Summary
- Root/worktree と path model の runtime 契約を検証するテスト。root placeholder の解決、repo root と linked worktree の区別、process-global cwd を扱う pushd の排他、run worktree の作成・検索・削除における管理領域・branch・Git 登録・symlink・path component の安全性を確認する。

## Read this when
- path model や root placeholder の解決挙動を変更・調査するとき
- linked worktree、run root、managed worktree の作成・検索・削除処理を変更・調査するとき
- pushd の並列実行時の cwd 競合対策を変更・調査するとき
- Git global config に依存しない test repository 作成や、worktree path の安全性を検証するとき

## Do not read this when
- CLI の通常のサブコマンド処理や、path model・worktree runtime と無関係な機能を変更・調査するとき
- run worktree の具体的な実装ではなく、別のテスト対象の入出力契約だけを確認するとき

## hash
- edec4bce76e3692f6b2b7a6296f09a973b008677e02a280251a328432a08d848
...
## Summary
- Codex subprocess の process group・pidfd・tracking 情報を検証するテスト群。SIGTERM の遅延配信、leader 終了後や割り込み時の descendant tracking 維持、tracking 失敗時の停止・reap、継承環境変数の無視、leader 同一性未確認時の fail-closed 動作を扱う。関連する runtime 実装の回帰テスト入口。

## Read this when
- Codex subprocess の起動、追跡対象 process group、子プロセス cleanup、pidfd を用いた signal、安全な PGID 停止の挙動を変更・調査するとき。
- editing_run の oracle 仕様に対する runtime subprocess のテストカバレッジを確認するとき。

## Do not read this when
- Codex runtime subprocess や process group cleanup の挙動に関係しない機能を調査するとき。
- 実装の詳細を確認したい場合。このファイルではなく、参照されている runtime モジュールを直接読むべき。

## hash
- 0b22849d5ca023ec2564c05f00d7ab3cf4b4fb10e0c8b90b3fd07bf4a736f91e
...
## Summary
- doctor preprocess の外部契約を検証する統合テスト。CLI と直接呼び出しの両方について、Git 状態・config・refactor state・lock・linked worktree の修復 lifecycle と、既存の staged/unstaged 差分や index flag の保持を確認する。doctor preprocess の挙動や回帰を調査する際の入口。

## Read this when
- doctor preprocess の実装・仕様変更によるテスト影響を確認するとき
- doctor が Git index、staged/unstaged 差分、rename、intent-to-add、index flag を保持できるか調査するとき
- repository と linked worktree 間の doctor lock、config 生成・同期、修復 commit の挙動を確認するとき

## Do not read this when
- doctor preprocess と無関係な CLI 機能や通常の Git 操作を調査するとき
- doctor の内部 helper 実装や正本仕様そのものを確認したいときは、対応する実装または oracle 文書を直接読む

## hash
- d80f98f9a843e847a50471869fa8f33510f851123a678232b73969a8a81782a6

# `test_editing_run_cli.py`

## Summary
- workload fork と共通 run join/abandon の統合 realization test。editing run の session state、run worktree、fork report、変更 path、INDEX 更新、Codex process tracking、rollback、join/abandon cleanup、refactor cycle の完了・中断・失敗時挙動を横断的に検証する。

## Read this when
- realization apply fork または realization refactor fork の lifecycle、state 遷移、report、rollback、INDEX 更新、process tracking を変更・検証するとき
- run join または run abandon の merge、cleanup、競合解決、失敗時復旧を変更・検証するとき
- fork と join/abandon が共有する worktree・branch・state の統合挙動を確認するとき

## Do not read this when
- 単一の CLI サブコマンド内部や個別 helper の実装だけを変更し、その lifecycle 統合挙動を確認する必要がないとき
- Codex 呼び出し、INDEX builder、refactor state などの単独機能の詳細を直接確認する場合は、それぞれの実装・専用テストを先に読むとき

## hash
- 2898f4d3e6afbaf1c54aac67cd8f545f6861b39b2fe23013a575dd000fd2035f
...
## Summary
- oracle review の report 生成・表示と CLI 出力を検証する回帰テスト。所見の受理・却下、severity 別分類、件数、評価対象 path、scope、Structured Output 呼び出し、timestamp 衝突回避、YAML frontmatter のエスケープを扱う。
- 正常終了だけでなく、ユーザー中断や列挙・judge 処理の失敗時に、完了済み対象だけを report に反映し、error summary と終了結果を適切に出力する契約を検証する。

## Read this when
- oracle review サブコマンドの report schema、所見表示、CLI 出力、エラー・中断時の挙動を変更または調査するとき
- oracle review の所見分類・件数集計、評価対象 path の扱い、report 保存処理を変更するとき
- oracle review 関連のテスト失敗が report 内容や CLI 出力に関係するとき

## Do not read this when
- oracle review の内部列挙・検証・judge ロジックそのものを変更・調査する場合は、まず対応する実装や専用テストを読む
- oracle review と無関係な CLI、report、oracle ファイル検証の作業を行う場合

## hash
- 690c5e29210ceb2a3e7dc56d98600a0aca1fb71913fef02555f366010ca893ad
...
## Summary
- oracle review の隔離 worktree lifecycle と INDEX.md 統合を検証する回帰テスト。linked worktree・snapshot commit からの review fork、preflight commit、未コミット差分や不正な非 INDEX 差分の拒否、割り込み・例外時の cleanup、INDEX conflict 解決、cleanup 失敗の報告を扱う。review worktree と INDEX 差分の外部契約を確認する入口。

## Read this when
- oracle review の worktree、session branch、snapshot commit、preflight、merge または cleanup の挙動を変更・調査するとき
- oracle review が INDEX.md のみを統合する制約や、差分検証・conflict 解決を確認するとき
- oracle review lifecycle に関する回帰テストの追加・修正・失敗調査をするとき

## Do not read this when
- oracle review の通常の finding 列挙・検証・判定ロジックだけを調査するときは、review 本体の実装や専用テストを直接読む
- INDEX.md の生成規則だけを調査するときは、indexing の仕様・実装・専用テストを直接読む
- session fork や一般的な run isolation の挙動だけを調査するときは、それぞれの専用テストを直接読む

## hash
- 69c3932ba26de17d0608494b1da25b5dba455fac43c6e2202f86b67016fa39dd
...
## Summary
- CmocConfig の既定値、JSON 化時のメンバー順、設定ファイルの読み込み失敗、および config_from_dict の入力検証を検証するテスト。
- Codex のモデル・provider・reasoning effort 設定、provider-local の JSON/TOML 値、recovery 試行回数の保持と永続化境界を扱う。

## Read this when
- cmoc config の既定値、シリアライズ順、読み込みエラー、設定値の型・値検証を変更または確認するとき。
- Codex model provider や model spec、reasoning effort、oracle review 設定の入力不正時の挙動を確認するとき。

## Do not read this when
- 設定値の実装詳細そのものを変更・調査する場合は、先に oracle の設定定義や runtime config 実装を読む。
- 設定以外の CLI 制御や一般的なエラー処理だけを確認する場合。

## hash
- a7b9e64fe91718f7050b266b0a603ce7e77369f3d46e3e6ce92eb6212378495d
...
## Summary
- realization refactor の永続 state と対象 file 管理を検証するテスト。oracle・realization file 集合の同期、調査履歴の保持、変更 file の再調査化、対象選択順、state schema 検証、path escape・symlink・Gitlink・非 UTF-8 などの境界条件を扱う。refactor state や対象 file classifier の挙動を確認する際の入口。

## Read this when
- refactor state の同期・読み書き・schema 検証を変更または調査するとき
- oracle/realization file の分類、path 安全性、対象選択規則を変更または検証するとき
- 永続 state の履歴保持や変更 file の再調査化を確認するとき

## Do not read this when
- INDEX.md の生成やルーティングだけを確認するとき
- refactor state、対象 file 分類、対象選択に関係しない機能のテストを調査するとき
- 実装の詳細を直接確認する必要があり、対応する runtime_refactor 実装を読むべきとき

## hash
- d3601b6a619e145360f40ccf3c62d54fffb69a5044bcec4b4ce740d08e74d4b9
