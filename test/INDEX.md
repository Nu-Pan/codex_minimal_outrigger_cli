...
## Summary
- doctor preprocess の外部契約を検証する統合テスト。Git 状態、config、refactor state、共有 doctor lock、linked worktree、symlink 安全性、修復 commit、および既存の staged・unstaged index 状態の保持を、CLI 呼び出しと直接呼び出しの両方で確認する。doctor の lifecycle や修復処理の回帰テストを探す入口。

## Read this when
- doctor preprocess の挙動、修復順序、失敗時の index 復元を変更または調査するとき
- doctor が config・agents・Git ignore・refactor state を修復する仕様をテストから確認するとき
- linked worktree、共有 lock、staged changes、index flag、rename の保持に関するテストを追加・修正するとき

## Do not read this when
- doctor preprocess と無関係な CLI サブコマンドや通常の doctor 実装を調査するとき
- Git 操作の共通 fixture や CLI テスト補助関数そのものを変更するときは、対応する共有 support module を先に読む
- 正本仕様の詳細を確認する必要があるときは、このテストではなく指定された oracle 文書を読む

## hash
- dd62984139f8111b85633e70b546dead766508e84f2da1e7c1c3df45f07e6d7b

# `test_editing_run_cli.py`

## Summary
- workload fork と共通 run join/abandon の統合 realization test。apply/refactor fork、run の state・branch・worktree lifecycle、Codex child tracking、INDEX 更新、report 生成、merge・cleanup・rollback・recover・interrupt・異常系を、隔離 Git repository を使った CLI 統合テストで検証する。

## Read this when
- realization apply/refactor fork の実装や fork report、agent 差分・INDEX 更新・process tracking を変更または調査するとき
- run join/abandon、session state、run worktree・branch の作成・merge・cleanup・rollback・recovery を変更または調査するとき
- editing run lifecycle の異常系、中断、競合、管理対象外差分、state 同期の挙動を検証するとき

## Do not read this when
- INDEX 生成単体、refactor state のデータ形式単体、または個別 helper の局所実装だけを確認する場合
- CLI の fork/join/abandon や editing run lifecycle に関係しない機能を変更・調査するとき

## hash
- c43df8ab380992903271cf75b9bfb54af1a1c6f5270ab55386cf4577d1d1e339
...
## Summary
- `commons.indexing` の INDEX entry 生成・解析・更新と directory traversal を直接検証する回帰テスト群。入力 schema 検証、hash による entry 再利用・再生成、空ディレクトリ、安定順序、並列更新、logger 伝播、cwd lock、memo 除外、symlink cycle・特殊ファイル・INDEX symlink の扱い、linked worktree 間の lock 共有を扱う。indexing runtime の共通契約を検証するテスト入口。

## Read this when
- INDEX entry の render/parse/update、hash 再利用、更新対象 directory の traversal 方針を変更・調査するとき
- INDEX 更新の並列実行、logger 伝播、pushd 中の worker 制約、lock 共有を変更・調査するとき
- symlink、特殊ファイル、memo directory、空 directory の INDEX 更新挙動を確認するとき

## Do not read this when
- CLI lifecycle や indexing サブコマンド全体の統合挙動を確認したいときは、対応する lifecycle・subcommand テストを直接読む
- INDEX entry の正本 schema や prompt 生成規則を確認したいときは、参照される oracle file を読む
- indexing と無関係な CLI、ログ、Git 操作のテストを調査するとき

## hash
- b87e9aecc9c6e05489f9a2f803257001f30ae097a9bc2733b6bfd5dec886c252
...
## Summary
- oracle review の review worktree lifecycle と INDEX.md 統合を検証する回帰テスト。linked worktree・snapshot commit からの fork、run target 衝突、割り込みや例外時の cleanup、未コミット差分の拒否、active editing run 中の実行、INDEX.md のみの統合、preflight commit、merge conflict 解決、Git が quote するパス、非 INDEX 差分の拒否、cleanup 失敗検出までを扱う。oracle review の worktree 隔離・差分制約・INDEX 統合の挙動を確認する入口となる。

## Read this when
- oracle review の worktree 作成・fork 元・branch lifecycle を変更または調査するとき
- oracle review の INDEX.md 統合、preflight commit、merge conflict 解決、差分制限を変更または調査するとき
- review run の割り込み・例外・cleanup 失敗時の挙動を検証するとき

## Do not read this when
- oracle review の所見列挙や判定ロジックだけを変更・調査し、worktree lifecycle や INDEX 統合に関係しないとき
- 通常の session fork、editing run、INDEX 生成の単独挙動を確認する場合は、それぞれの直接の実装・テストを先に読むとき

## hash
- c6de11bbfffc58616e1c75637cfc105c894039ee75c57b53193d5d4dcca34754
...
## Summary
- session/run state の永続化スキーマ、branch からの session ID・state 解決、JSON 読み書き、入力検証を検証する realization test。
- 不正な state payload・未定義 field・不完全な session identity・symlink 経由の読み書きを拒否する挙動を扱う。
- session fork lock が process・thread 間で共有されることも検証する。

## Read this when
- session state や run state の schema 検証を変更・調査するとき
- managed branch の解析、state file の読み込み・書き込み、session fork lock の実装を変更・調査するとき
- runtime state 関連のエラー処理や symlink 対策を検証するとき

## Do not read this when
- CLI の通常の session/run 操作や branch 作成処理だけを調査するとき
- runtime state の実装詳細ではなく、別機能のテストや仕様を確認するとき

## hash
- 1a7fa4fa36818a9e6e5cf9b070072e620390e60a756c6b21b66d725de1eb04d8
