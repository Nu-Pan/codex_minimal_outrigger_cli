...
## Summary
- Root/worktree と path model の runtime 契約を検証するテスト。placeholder の解決、repo root と linked worktree の区別、process-global cwd を扱う pushd の排他、run worktree の作成・検索・削除における管理領域・branch 対応・Git 登録・symlink・置換 path の安全性を対象とする。

## Read this when
- path placeholder、repo root、linked worktree、work root の runtime 挙動を変更または調査するとき
- run worktree の作成・検索・削除、管理 path の検証、symlink 対策を変更または調査するとき
- pushd の並行利用や process-global cwd の競合を変更または調査するとき

## Do not read this when
- runtime path や worktree の安全性に関係しない CLI 機能・モデル処理・別領域のテストを扱うとき
- path model や worktree API の実装詳細ではなく、単一の別テスト対象の挙動だけを確認するとき

## hash
- d84cfe58f00b664dd386048a8b2c25d1b415034c7adf096fdb4dcbf1aa189fb0
...
## Summary
- Codex subprocess と run process の停止・追跡・cleanup に関するテストを集約する。pidfd、process group、tracking file、signal、異常系での fail-closed 挙動を検証し、runtime_codex_profile と runtime_run の実装を読む入口となる。

## Read this when
- Codex subprocess の起動・追跡、SIGTERM 配信、process group の停止や再利用検知を変更・調査するとき。
- run process tracking file の読み書き、壊れた状態・特殊ファイル・不正な process identity の扱いを変更・調査するとき。
- leader 終了後の descendant cleanup、pidfd unavailable、tracking 更新失敗、interrupt 時の child 保持を検証するとき。

## Do not read this when
- Codex subprocess や run process の cleanup、tracking、process group 停止に関係しない機能を扱うとき。
- 実装の詳細を確認する必要があり、対応する runtime モジュールを直接読む方が適切なとき。

## hash
- 1db1dc4b87242063c52452bac34f655267280638ffdfcece86304324b48d8a8f
...
## Summary
- workload fork と共通 run join/abandon の統合 realization test。editing run の session state、run worktree、fork report、Codex child tracking、INDEX refresh、rollback、merge、cleanup、interrupt、error recovery など、apply/refactor fork と run lifecycle の状態遷移を横断的に検証する。

## Read this when
- realization apply fork または realization refactor fork の lifecycle、state 遷移、report 生成、worktree/branch cleanup を変更・調査するとき
- run join または run abandon の merge、force-resolve、競合復旧、cleanup、process tracking を変更・調査するとき
- INDEX refresh や Codex child tracking が editing run の開始・処理単位 commit・joinable 公開・join/abandon cleanup とどう連携するか確認するとき
- fork、join、abandon、interrupt、rollback、異常終了時の統合挙動を検証するとき

## Do not read this when
- 個別の実装関数だけを変更・調査し、その lifecycle 統合や外部 CLI 挙動への影響がないとき
- 単独の report 表示形式、refactor state schema、INDEX builder の内部実装だけを対象とし、editing run の lifecycle を確認する必要がないとき

## hash
- f4ea76edee33931796c26cc8fbff8017f9b19688e881004c87b12f6a5358ed40
...
## Summary
- Codex 呼び出し前の indexing preflight を検証するテスト。exec/TUI 経路での実行順序、対象 worktree の選択、repository lock 待機、パラメータによる無効化、file access violation 後に recovery indexing を行わない制約を確認する。

## Read this when
- Codex 呼び出し前の indexing preflight の実行条件・順序を変更または調査するとき
- linked worktree、repository lock、preflight 無効化、file access violation の挙動を変更または検証するとき
- indexing と Codex exec/TUI の連携テストの失敗原因を調べるとき

## Do not read this when
- indexing 自体の生成・更新ロジックだけを変更するとき
- Codex CLI の一般的な実行処理だけを変更し、preflight 連携に影響しないとき
- indexing や preflight と無関係なテスト・機能を扱うとき

## hash
- 79e6a972ef55465b27cc2f00e773782585a57e34e3fdd7176333aed9680d3d62
...
## Summary
- oracle review の review worktree lifecycle を検証する回帰テスト群。linked worktree と snapshot commit からの隔離、run target 衝突、中断・例外時の branch/worktree cleanup、未コミット差分の拒否、INDEX.md のみの統合と merge conflict 復旧、不要な差分や cleanup 失敗の報告を扱う。oracle review の worktree 隔離・INDEX 統合仕様に対応するテストの入口。

## Read this when
- oracle review の review branch・linked worktree・run worktree の作成元や隔離を変更するとき
- oracle review における snapshot、preflight commit、INDEX.md の統合・merge conflict 解決を変更するとき
- KeyboardInterrupt、例外、cleanup 失敗時のリソース復旧やエラーレポートを変更するとき
- review worktree が許可される差分範囲や未コミット変更の検証を変更するとき

## Do not read this when
- oracle review の通常の所見判定ロジックや Structured Output schema 自体だけを変更するときは、まず review 実装と対応する oracle 仕様を読む
- INDEX.md の生成アルゴリズムだけを変更するときは、まず indexing 実装・仕様と専用テストを読む
- review worktree lifecycle、INDEX.md 統合、cleanup の回帰挙動に関係しない CLI 機能を変更するとき

## hash
- 9a670233977daf3dd16fc15e3898bd9cf2d2e650fc9f10ae76e6688895ee992f
