...
## Summary
- プロンプト部品とパラメータビルダーのテスト群であり、標準文書レンダリング、ルーティング規則、ファイルアクセス規則、完全プロンプトへの標準注入、用語置換、TUI パラメータ選定、各ビルダーのモデル・推論強度・アクセスモードを検証する。
- 構造化ドキュメントの Markdown レンダリングや、正本仕様・編集対象・レビュー・INDEX エントリー関連の基準文言が期待どおり含まれること、また不要な標準や旧用語が出力に混入しないことを確認する入口になる。

## Read this when
- プロンプト生成、標準文書生成、ルーティング規則、ファイルアクセス規則、または完全プロンプトの構成を変更し、その出力文言や含まれる標準の有無を確認したいとき。
- Codex CLI 向けの用語置換、コードブロック内文字列の扱い、リポジトリルート解決、または TUI 実行パラメータ選定の schema・既定値に関わる挙動を調べるとき。
- apply fork、indexing、review oracle、session join などの各パラメータビルダーが、期待されるモデルクラス・推論強度・ファイルアクセスモード・プロンプト断片を使っているか確認するとき。
- 構造化ドキュメントの Markdown レンダリングで連続空行の畳み込みやタイトル出力のような基本整形を変更する前後に、既存期待値を把握したいとき。

## Do not read this when
- 個別のプロンプト部品や標準文書の実装内容そのものを修正する場合で、テスト期待値ではなく生成ロジックを直接確認すべきとき。
- CLI コマンド実行、永続状態、Git 操作、ワークツリー管理など、プロンプト生成・パラメータ選定と無関係な挙動を調べるとき。
- 特定の builder の公開 API や内部実装を詳細に追う必要があり、テストで確認される外部期待だけでは判断できないとき。

## hash
- bd8a05584e614f56bf071c7d266c2edbf04faef8f883b3e4cd1bb70fcdc3ac2e
