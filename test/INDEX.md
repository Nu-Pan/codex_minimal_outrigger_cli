...
## Summary
- プロンプト部品とパラメータビルダーの挙動を検証するテスト群。レビュー基準、ルーティング規則、ファイルアクセス規則、各種標準文書、完全プロンプトへの挿入と用語書き換え、TUI パラメータ解決、モデル種別・推論努力・アクセスモードの選定を扱う。
- 構造化ドキュメントの Markdown 描画、連続空行の折り畳み、コードブロック内を除いた文言置換、リポジトリルート基準のプロンプト生成など、プロンプト生成の利用者向け出力と内部制御値の両方を検証する入口になる。

## Read this when
- プロンプト部品の生成結果に含めるべき見出し・用語・基準文言を確認または変更するとき。
- 完全プロンプトがルーティング規則、レビュー基準、仕様文書基準、編集対象ファイル保守基準、INDEX.md エントリー基準をいつ含めるかを確認するとき。
- Codex CLI 向けに置換される用語と、コードブロック内では置換しない境界を確認するとき。
- ファイルアクセスモードごとの制約文言、TUI パラメータ解決の schema、モデルクラス、推論努力、アクセスモードの期待値を変更するとき。
- apply fork、review oracle、session join などのビルダーが生成するパラメータやプロンプト断片の基本契約を調べるとき。

## Do not read this when
- 個別プロンプト部品の実装ロジックそのものを変更したいだけで、期待される外部挙動やテスト契約を確認する必要がないとき。
- CLI コマンド実行、Git 操作、永続状態管理など、プロンプト生成以外の挙動を調べたいとき。
- 仕様文書の正本内容を確認したいとき。この対象は生成済みプロンプトが期待文言を含むかを検証するテストであり、正本仕様そのものではない。

## hash
- 2c730703c0569a01be4c3e053f367d96e67749065d05adc3dcd4f48106673be5
