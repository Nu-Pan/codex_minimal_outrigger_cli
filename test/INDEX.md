...
## Summary
- プロンプト部品と実行パラメータ生成のテスト群。ルーティング規則、ファイルアクセス規則、各種標準文書の描画、完全プロンプトへの注入・用語置換、インデックス作成・レビュー・セッション合流などのパラメータ属性を検証する。
- 構造化ドキュメントの Markdown 描画、空行圧縮、コードブロック内テキストを保護した置換、リポジトリルート解決を含むプロンプト生成の期待挙動を確認する入口になる。

## Read this when
- プロンプト部品の本文、タイトル、Markdown レンダリング結果、または完全プロンプトに含まれる標準文書の有無を変更・確認したいとき。
- ファイルアクセスモードごとの読み書き制約文、TUI パラメータ選定、インデックスエントリー生成、レビュー所見統合、セッション合流などのビルダーが返すモデル種別・reasoning effort・アクセスモードを確認したいとき。
- 用語置換やパス置換が通常文では行われ、コードブロック内では保持される挙動を調査・修正したいとき。
- プロンプト生成や標準文書生成の変更により、既存テストの期待文字列を更新すべきか判断したいとき。

## Do not read this when
- 個別コマンドの実処理、ファイル操作、Git 操作、永続状態管理など、プロンプト生成以外のランタイム挙動を調べたいとき。
- 標準文書やプロンプト部品の実装そのものを変更したい場合で、まず対象ビルダー本体を読む方が直接的なとき。
- TUI やレビューやセッション合流の仕様本文を確認したいだけで、生成されたプロンプトやパラメータのテスト期待値を確認する必要がないとき。

## hash
- ba454f2f7910c5e8be6c3644b064275203d45623cee7845d7c0ab76ea6dde63d
