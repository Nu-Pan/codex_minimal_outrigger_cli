...
def install_quota_probe_oracle_builder(monkeypatch: pytest.MonkeyPatch) -> None:
    """テスト中だけ正本 quota probe builder の契約を提供する。"""
    ...
    def build_oracle_parameter(
        ...
        """quota probe の canonical builder を模擬する。"""
        return AgentCallParameter(
            ModelClass.MINIMUM,
            ReasoningEffort.LOW,
            FileAccessMode.READONLY,
            "oracle quota probe prompt",
            None,
...
def test_quota_probe_adapter_delegates_to_oracle_builder() -> None:
    """adapter が正本 builder の返却値をそのまま返す。"""
    ...
    assert probe.prompt == "oracle quota probe prompt"
    assert probe.structured_output_schema_path is None
    assert probe.run_indexing_preflight is False
    assert probe.cwd == base.cwd


def test_quota_probe_adapter_requires_oracle_builder(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """正本 builder 欠落時に realization 側 fallback を作らない。"""
    monkeypatch.delitem(sys.modules, "oracle.acp_builder.quota_probe")
