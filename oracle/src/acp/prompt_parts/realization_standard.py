...
def build_realization_standard() -> StructDoc:
    ...
    standards = [
        Standard(
            ...
            examples=[
                "複数箇所で出現する同じ path 正規化処理は 1 つの helper に集約する",
                "仕様変更により使われなくなった旧分岐と対応テストは削除する",
                "ほぼ同じ処理を別名の関数として複数箇所に残してはいけない",
                "文字数を減らすために、意味が読み取りにくい過度な短縮名を使ってはいけない",
            ],
        ),
        Standard(
            ...
            examples=[
                "コードの意味的なブロックごとに、その実装でなければならない理由を短いコメントで補う",
                "仕様変更により使われなくなった互換分岐は削除する",
                "「将来別の形式にも対応するかもしれない」という理由だけで汎用 parser 層を追加してはいけない",
                "シグネチャを見れば分かる引数名や戻り値型だけを docstring で長く説明してはいけない",
            ],
        ),
        Standard(
            ...
            examples=[
                "新しい parser を追加する時に、旧 parser を呼ぶ経路・旧 parser 専用テスト・旧 parser 専用 fixture も整理する",
                "既存関数に引数を 1 つ追加して現行仕様を満たせるなら、その形で既存関数を使う",
                "既存関数を少し変えれば足りる処理のために、ほぼ同じ関数を別名で作ってはいけない",
                "`legacy_` や `old_` を名前に含む実装を、現行仕様上の必要性が不明なまま残してはいけない",
            ],
        ),
        Standard(
            ...
            examples=[
                "同じ入力・同じ失敗時挙動を持つ path 変換処理は 1 つの helper にまとめる",
                "1 モジュール内でだけ使う補助処理は private helper として置く",
                "2 つの処理がどちらも git command を呼んでいるという理由だけで、失敗時の扱いが違う処理を 1 関数にまとめてはいけない",
                "`do_everything(options: dict)` のような万能関数で差分を吸収してはいけない",
                "1 箇所からしか呼ばれず、名前を付けても意図が増えない 3 行の処理を無理に関数化してはいけない",
            ],
        ),
        Standard(
            ...
            examples=[
                "現行仕様で利用者が選ぶ必要のある挙動だけを CLI option として追加する",
                "新しい状態ファイルを追加する時に、作成・更新・削除の条件も実装上明確にする",
                "デバッグ用に一時的に欲しい値を、恒久的な CLI option として追加してはいけない",
                "計算で再現できる値を、別ファイルに永続化してはいけない",
                "仕様上 1 つの動作で十分な処理に `--mode` や `--strategy` を追加してはいけない",
            ],
        ),
        Standard(
            ...
            examples=[
                "同じ関数の入力値違いの正常系は 1 本の parametrized test にまとめる",
                "内部 helper の呼び出し順ではなく、仕様上の出力や副作用を検証する",
                "同じ失敗条件を複数のテストで重複して検出してはいけない",
                "Codex CLI に依頼した仕事の出力品質を自動テストで検証してはいけない",
                "小さな入力で足りるテストのために巨大な fixture ファイルを追加してはいけない",
            ],
        ),
        Standard(
            ...
            examples=[
                "標準ライブラリでは安全に実装しにくい処理のために、十分に小さく一般的な依存を追加する",
                "テストで必要な小さな入力は、巨大な fixture ファイルではなくテスト内で組み立てる",
                "数行の path 操作のために外部パッケージを追加してはいけない",
                "実行すれば再生成できるファイルを、正本のようにコミットしてはいけない",
                "既存のテストコマンドで足りる処理のために、別の補助スクリプトを追加してはいけない",
            ],
        ),
        Standard(
            ...
            examples=[
                "実装の途中で作った確認用 helper は、最終的に使わないなら削除する",
                "新しい共通関数へ移行した後、旧 helper の import とテストも削除する",
                "新実装への移行後に、呼ばれなくなった旧 helper と旧 helper 専用テストを残してはいけない",
                "仕様説明に寄与しない「念のため」コメントを残してはいけない",
            ],
        ),
    ]
    return StructDoc(
