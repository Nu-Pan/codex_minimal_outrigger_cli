...
## Summary
- AI エージェントへ渡す「file read write rule」プロンプト断片を、読み書きモードプリセットごとに構築する実装。
- work root を解決し、readonly、oracle 読み取り専用、realization 編集、oracle 編集の各モードに応じて、ツリー外アクセス、oracle、memo、リポジトリ書き込みの禁止条件を本文として組み立てる。
- 不正なモードを拒否し、構築した本文を構造化ドキュメントとして返す入口を担う。

## Read this when
- ACP 向けプロンプトに含めるファイル読み書き制約の文面を確認または変更したいとき。
- 読み書きモードプリセットごとに、どの領域の読み込み・書き込みを禁止するかを確認したいとき。
- work root を基準にした oracle、memo、リポジトリ書き込み禁止のプロンプト生成ロジックを追いたいとき。
- ファイルアクセス制約を構造化ドキュメントへ変換する処理の責務や例外条件を確認したいとき。

## Do not read this when
- oracle file や realization file の概念定義そのものを確認したいだけのとき。
- パスキーワードや work root 解決規則そのものを確認したいとき。
- ファイルアクセス権限の実際の強制処理や OS レベルの制御を探しているとき。
- ACP プロンプト全体の構成や他のプロンプト断片の内容を確認したいとき。
