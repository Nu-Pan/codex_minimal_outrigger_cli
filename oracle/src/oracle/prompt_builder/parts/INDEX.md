...
## Summary
- ファイルアクセスプロファイルから、AI エージェント向けのファイル読み書き規則を表すプロンプト部品を構築する実装です。
- 各アクセス規則のパターンと属性を箇条書き本文に変換し、作業ルートのプレースホルダと構造化ドキュメントを返します。
- ファイルアクセス制御の正本データそのものではなく、既存のプロファイルをプロンプト文書へ変換する入口です。

## Read this when
- ファイルアクセスプロファイルをプロンプトに埋め込む処理を確認したいとき。
- ファイル読み書き規則の提示形式、プレースホルダ、構造化ドキュメント生成の関係を調べるとき。
- ファイルアクセス規則のプロンプト部品を組み立てる処理を変更するとき。

## Do not read this when
- ファイルアクセスプロファイルの規則定義や許可属性そのものを確認したいとき。
- 作業ルート解決やパス表記モデルの詳細を確認したいとき。
- プロンプト全体の組み立て順序や他のプロンプト部品との統合を調べたいとき。

## hash
- d362a1976122f0c798d4eb4b854098a6bc6f1acaab6879e3177c34613b12e71e
