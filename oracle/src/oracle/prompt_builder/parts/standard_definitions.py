...
ORACLE_INTENT_AND_GAPS_STANDARD = Standard(
...
ORACLE_CONSISTENCY_AND_SEARCHABILITY_STANDARD = Standard(
    standard_id="oracle.40.consistency_and_searchability",
...
REALIZATION_ORACLE_CONFORMANCE_STANDARD = Standard(
    standard_id="realization.10.oracle_conformance",
...
REALIZATION_CURRENT_SPEC_ONLY_STANDARD = Standard(
    standard_id="realization.20.current_spec_only",
...
REALIZATION_REPOSITORY_VERIFICATION_STANDARD = Standard(
    standard_id="realization.30.repository_verification",
...
FINDING_BASIS_EVIDENCE_STANDARD = Standard(
    standard_id="finding_basis.10.evidence",
    title="所見・修正対象に具体的な根拠を求める",
    required=(
        "所見または修正対象には、用途固有 Standard が認める具体的な oracle file または realization file の記述・挙動を示す",
    ),
    prohibited=(
...
APPLY_REVIEW_FIX_TARGETS_STANDARD = Standard(
    standard_id="apply_review.10.fix_targets",
...
APPLY_REVIEW_ALREADY_RESOLVED_STANDARD = Standard(
    standard_id="apply_review.20.already_resolved",
    title="調査開始時点で解消済みの問題を所見にしない",
    prohibited=("調査開始時点ですでに解消されている問題を所見として扱ってはいけない",),
)

ORACLE_REVIEW_FATAL_STANDARD = Standard(
    standard_id="oracle_review.10.fatal",
...
ORACLE_REVIEW_MINOR_STANDARD = Standard(
    standard_id="oracle_review.20.minor",
...
ORACLE_REVIEW_ORACLE_ONLY_STANDARD = Standard(
    standard_id="oracle_review.30.oracle_only",
...
CONFLICT_RESOLUTION_PRESERVE_BOTH_BRANCHES_STANDARD = Standard(
    standard_id="conflict_resolution.10.preserve_both_branches",
...
INDEX_ENTRY_EVIDENCE_STANDARD = Standard(
    standard_id="index_entry.20.evidence",
...
INDEX_ENTRY_SEMANTIC_INFORMATION_STANDARD = Standard(
    standard_id="index_entry.30.semantic_information",
