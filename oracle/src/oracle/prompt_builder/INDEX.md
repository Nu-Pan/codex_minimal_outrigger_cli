...
## Summary
- エディタ経由で後続 AI エージェントへ渡すプロンプト入力ファイルの初期表示文面を構築する定義。使い方・記入目安・完全プロンプトの挿入先をコメントブロック内に組み立てるため、エディタ入力の初期テキスト生成やその構成を確認・変更するときの入口となる。

## Read this when
- エディタ経由のプロンプト入力ファイルに表示する初期案内文面を変更するとき。
- 入力内容の完全プロンプトへの挿入位置や、初期文面の Markdown 構成を確認するとき。

## Do not read this when
- エディタ経由ではないプロンプト生成や、完全プロンプト全体のテンプレート定義を確認するとき。
- StructDoc・StructBlock の一般的な仕様や Markdown レンダリングの実装を確認するときは、それぞれの定義を直接読む。

## hash
- d01c01f33173c7fa33262a44f721590aaf28a545390c3ec8246df270d08c9230
