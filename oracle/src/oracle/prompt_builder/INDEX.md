...
## Summary
- エディタ経由で後続 AI エージェントへ渡すユーザー入力ファイルの初期テキストを構築する正本実装。利用方法、プロンプト記入ガイドライン、完全なプロンプトテンプレートを HTML コメント内に Markdown として生成する。

## Read this when
- エディタ経由のプロンプト入力ファイルに注入する初期内容の形式や構成を変更・確認するとき
- 初期テキストに含める利用説明、記入ガイドライン、プロンプトテンプレートの生成処理を変更・確認するとき
- StructDoc、StructBlock、render_as_markdown を用いたこの初期テキスト生成の責務を確認するとき

## Do not read this when
- エディタ経由ではないプロンプト生成や、後続エージェントへ渡す完全プロンプトの別の構築処理を確認するとき
- ユーザー入力ファイルの編集後の解析・検証処理を確認するとき
- Markdown 構造化レンダリング自体の仕様や実装を確認するときは、構造化文書関連の実装を直接読む

## hash
- a6081dffe8d14bd06f8efbaba3f3e9864d1c7b944f221c0bed866a1a6d00042c
