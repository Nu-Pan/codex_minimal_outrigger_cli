...
def build_realization_policy(
    ...
    return (
        ...
        SDHeader(
            "realization policy",
            SDPolicy(
                what_is_this="realization file が満たすべき規定を以下に示す",
                require=(
                    "oracle file を **人間意図を具体化した正本仕様断片** として扱う",
                    "oracle file で明示されていない仕様の隙間は、oracle file で明示されている要求を満たすために必要となる範囲内で、実装者の裁量で realization file として補完する",
                    ...
                    "エージェントから参照可能な文章上で指示されている手順に基づいて、作業後の状態が検証・テストに合格する状態であること",
                    "検証・テストを何らかの理由で実施できない場合は、検証済みと扱わず不足を報告する",
                    "realization file のコメントに、その実装の根拠となる oracle file への参照 (パス・行・簡潔な内容) を書くこと",
                ),
                prohibit=(
