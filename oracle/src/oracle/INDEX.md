...
## Summary
- cmoc の正本実装のうち、設定、パス表記モデル、規範文書モデル、構造化 Markdown レンダリング helper など、複数領域から参照される基礎的な補助概念を集める領域。
- リポジトリ別設定の既定値と同期方針、ルートプレースホルダ付きパスの解決・逆変換、規範を構造化して扱うためのデータ構造、階層文書を Markdown へ整形する処理への入口になる。

## Read this when
- cmoc 全体で共有される設定値、パス表記規則、規範文書の構造化、または Markdown レンダリング helper の正本実装を探すとき。
- Codex CLI に渡すモデル関連設定、ファイルアクセス規則違反時のリカバリ回数、apply fork や review oracle の上限値など、横断的な設定の根拠を確認したいとき。
- <cmoc-root>、<repo-root>、<run-root>、<work-root> 相当のルート概念、プレースホルダ付きパスと絶対パスの相互変換、相対パス禁止規則を確認・実装・検証するとき。
- 規範を背景・要求・判断例などの構造に分けて保持するモデル、または構造化された自然言語文書を Markdown として出力する helper の挙動を確認したいとき。

## Do not read this when
- 個別サブコマンドの利用者向け入出力、実行フロー、状態ファイルの仕様を探しているときは、そのサブコマンドの仕様または実装へ進む。
- INDEX.md のルーティング規則、oracle file と realization file の管理方針、または実現ファイルに適用される品質基準そのものだけを確認したいときは、対応する規範文書へ進む。
- 設定ファイルの読み書き処理、JSON 変換処理、init 処理など、正本値ではなく realization implementation 側の具体的なアルゴリズムだけを確認したいときは、対応する実装へ進む。
- 生成済み Markdown の内容や配置先、個別の規範本文、CLI の実行状態など、基礎 helper 以外の具体的な仕様を調べているとき。

## hash
- e0e9577168f329fca5f4d4fab0a6c0e31c94a13ed9e98492196456dfa1ee045b
