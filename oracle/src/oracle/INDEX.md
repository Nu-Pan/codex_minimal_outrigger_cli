...
## Summary
- cmoc の設定モデル、パスコンテキスト、構造化文書の Markdown レンダリングを担う基盤モジュール群への入口。
- 設定値の既定値や Codex 呼び出し設定、プレースホルダ付きパスの解決、文書ノードの構造化と出力を扱う。

## Read this when
- cmoc のリポジトリ固有設定、Codex provider・agent call 設定、または設定の永続化対象を確認するとき。
- agent call の cwd から worktree・repository ルートを導出する規則や、ルートプレースホルダの解決を調べるとき。
- 見出し、タグ付きブロック、コードブロック、構造化ポリシーを Markdown に変換する仕組みを確認するとき。

## Do not read this when
- 特定の agent call、CLI 機能、oracle の調査・編集・還元処理そのものを調べるとき。
- 設定値のシリアライザ実装や config.json 生成処理だけを確認したいとき。
- パスモデルや文書ノードを介さない個別機能の実装を直接調べれば足りるとき。

## hash
- f18a52094663670465b0cd37efe4dfd8a40867e3be0806665d369215e1b711a7
