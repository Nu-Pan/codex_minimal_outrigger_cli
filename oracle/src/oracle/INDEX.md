...
## Summary
- cmoc の設定、パス表記、規範文書モデル、構造化 Markdown レンダリングなど、横断的な正本実装断片を集めた領域。
- リポジトリ別設定の既定値、ルートプレースホルダの解決、規範を構造化して扱うためのデータ構造、階層文書を Markdown 化する helper への入口になる。

## Read this when
- cmoc の設定項目、既定値、Codex CLI 向けモデル・推論設定、apply fork や review oracle のループ上限を確認したいとき。
- <cmoc-root>、<repo-root>、<run-root>、<work-root> などのルート概念、プレースホルダ付きパスと絶対パスの変換規則、相対パス禁止の扱いを確認したいとき。
- 規範文書を背景・要求・判断例などの構造として保持し、構造化ドキュメントへ変換するモデルを確認したいとき。
- 階層化された文章、本文、コードブロックを Markdown としてレンダリングする helper の挙動を確認・変更したいとき。

## Do not read this when
- CLI サブコマンド固有の利用者向け入出力、実行フロー、状態ファイルの仕様を探しているとき。
- 設定値の正本ではなく、設定ファイルを実際に読み書きする realization implementation だけを探しているとき。
- 個別の規範本文や、実現ファイルに適用される品質基準そのものを確認したいとき。
- INDEX.md のルーティング規則や oracle file と realization file の管理方針だけを確認したいとき。

## hash
- 7327cd3792945db55a07596ed5a10a1d06b7bd6d609e3e58ab0895e8308c93a0
