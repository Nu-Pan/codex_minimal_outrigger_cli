...
## Summary
- cmoc の共通的な挙動・運用を定める正本仕様群をまとめたディレクトリ。自動補完、Codex CLI 呼び出し、ログ、feedback、prompt、session／run、サブコマンド、エラー・中断処理などの仕様への入口として機能する。

## Read this when
- 複数の共通仕様から、対象の責務に対応する正本文書を選ぶとき
- Codex 呼び出し、ログ、feedback、prompt、session／run、サブコマンドなど横断的な挙動の仕様を調べるとき
- 対象となる個別仕様がまだ特定できず、app_spec 配下の仕様群を案内として確認するとき

## Do not read this when
- 特定の仕様文書や実装の対象がすでに明確で、その本文へ直接進めるとき
- Python 環境、設計境界、テスト実行手順など開発規則を確認するときは、対応する dev_rule を読む
- 個別機能の realization 実装だけを確認し、正本仕様の選択や横断的な契約を調べる必要がないとき

## hash
- 58359829324c8c096e677784b4231b184b7eedc356b3b904028cb51f207fd22f
