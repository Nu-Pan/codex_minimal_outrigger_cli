...
## Summary
- cmoc のアプリケーション仕様をまとめたディレクトリ。CLI 自動補完、Codex 実行・provider、ログ、doctor 前処理、feedback、prompt、run/session lifecycle、サブコマンド、中断、Windows toast 通知などの正本仕様を扱い、各機能の挙動や責務境界を確認する際の入口となる。

## Read this when
- cmoc のアプリケーション仕様全体から、対象機能に対応する正本仕様を探すとき
- CLI 実行、ログ、feedback、prompt、run/session、サブコマンド、通知などの挙動・責務境界を実装またはレビューするとき
- 対象仕様が共通規則と個別仕様のどちらに定義されているか判断するとき

## Do not read this when
- 特定の実装ファイルやテストの具体的な挙動だけを確認したいとき
- 開発環境やテスト実行手順を確認したいとき
- 対象機能の詳細が専用の下位仕様や実装に直接定義されており、その本文だけで足りるとき

## hash
- d7d077784230746651e2827be3fc2c1ea858fc02ee221a539d7b8ca103c64b00
