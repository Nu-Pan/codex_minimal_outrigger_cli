...
## Summary
- cmoc のアプリケーション仕様を集約するディレクトリ。CLI 自動補完、Codex 実行契約、共通出力・エラー処理、doctor、feedback、session/run、indexing、通知など、cmoc の主要な挙動・状態・実行境界を定める正本仕様への入口を提供する。

## Read this when
- cmoc のアプリケーションレベルの挙動仕様、実行契約、状態遷移、出力・通知・feedback の責務分担を確認するとき
- 特定の仕様書が不明な場合に、サブコマンド、共通契約、feedback、session/run、Codex 呼び出し関連の正本仕様を探すとき

## Do not read this when
- 個別仕様書が明確で、その本文だけを確認すればよいときは、このディレクトリ全体ではなく該当する仕様書を直接読む
- 具体的な realization 実装、テスト手順、保存済み成果物の内容だけを確認するときは、対応する実装・テスト・成果物を直接読む

## hash
- 0b454fa18289f0e281797de25d0df2058daff84cee6b77d2dc082dfdf954da37
