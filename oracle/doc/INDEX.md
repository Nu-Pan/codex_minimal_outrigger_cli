...
## Summary
- cmoc のアプリケーション挙動に関する正本仕様群の入口。CLI 自動補完、Codex 呼び出し、ログ・エラー処理、doctor、feedback、prompt、run/session lifecycle、サブコマンド、通知など、実行時の機能契約を扱い、個別の挙動を確認する際は該当する下位仕様へ進むために読む。
- 共通仕様と個別機能仕様の責務境界を示し、出力・状態・中断・通知・外部呼び出しなど複数機能にまたがる契約を確認する起点となる。実装配置やテスト実行手順、個別仕様の詳細は、それぞれの設計・テスト規約または対応する下位文書へ委譲する。

## Read this when
- cmoc のアプリケーション仕様を調査・実装・レビューし、対象機能に対応する正本仕様を選ぶとき
- CLI サブコマンド、agent/Codex 呼び出し、ログ・エラー処理、feedback、prompt、run/session、通知、自動補完などの挙動契約を確認するとき
- 複数の機能にまたがる状態遷移、出力、report、終了処理、外部呼び出しの共通境界を確認するとき

## Do not read this when
- 対象の個別仕様が既に特定できており、その仕様本文を直接読む方が適切なとき
- realization implementation の配置や実装責務だけを確認するとき
- テストの設計・実行方法、開発環境、または INDEX.md の生成規約だけを確認するとき

## hash
- f78033ad1cd9a7275ae779b8fe4f61ad79fe81b0a2d258b215a11457a448f1fe
