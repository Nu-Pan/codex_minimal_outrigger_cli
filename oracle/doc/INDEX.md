...
## Summary
- oracle/doc/app_spec は、cmoc のアプリケーション仕様を分野別に確認するための正本文書群。CLI 自動補完、Codex 実行、ログ・エラー処理、doctor、feedback、prompt、run/session lifecycle、サブコマンド、通知などの共通契約と個別仕様への入口を提供する。対象の挙動仕様を実装・変更・レビューする際は、ここから該当する下位仕様へ進む。

## Read this when
- cmoc のアプリケーションレベルの挙動仕様、共通契約、サブコマンド仕様の入口を選ぶとき
- CLI 自動補完、Codex 呼び出し、ログ・エラー処理、doctor、feedback、prompt、run/session、通知などの正本仕様を調査するとき
- 個別仕様間の責務境界や、共通仕様から下位の専用仕様へ進む条件を確認するとき

## Do not read this when
- 対象の個別仕様が既に特定できており、そのファイルを直接読む方が適切なとき
- 実装配置、テスト実行、開発環境、oracle／realization の分類など、専用の設計・テスト・開発ルールだけを確認するとき
- INDEX.md の自動生成処理そのものや、個別の実装コード・テストコードの詳細だけを調査するとき

## hash
- 92bffd6df27e3c834f8c2eaf0affb5d62e4a499fd3faf3a6aa7f8123731db5ee
