...
## Summary
- cmoc の利用方法とアプリケーション動作に関する正本仕様群で、共通の実行規則、状態やファイルの扱い、agent 呼び出し、出力、feedback、indexing など横断的な契約を定める。
- 個々のサブコマンド固有の目的、引数、実行手順、結果の詳細は専用仕様に分担し、この文書群は共有動作とコマンド間の連携をたどる入口となる。

## Read this when
- cmoc の動作を変更・レビューするとき、複数のコマンドに関わる共通規則や、その要求を定める仕様を特定する。
- 前処理、run/session の状態、agent 呼び出し、入力 handoff、出力、feedback、indexing などの横断的な振る舞いや、個別仕様から参照される共通契約を確認する。

## Do not read this when
- 確認したい範囲が一つのサブコマンドの目的、引数、実行手順、固有の結果に限られる場合は、そのコマンドの専用仕様へ直接進む。
- 実装構造やテスト方法、リポジトリの開発規約だけを確認する場合は、それぞれを扱う資料へ進む。
- ブランチやコミットの一般モデルだけを確認する場合は、そのモデルを定める正本へ直接進む。
