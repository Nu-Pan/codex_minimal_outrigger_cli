...
## Summary
- cmoc のアプリケーション仕様群を、CLI 自動補完、Codex agent call、ログ・エラー処理、feedback、session／run、各種サブコマンドなどの責務別に案内する正本仕様の集合。個別機能の挙動、共通実行規約、状態管理、通知、利用手順を確認する際の入口となる。

## Read this when
- cmoc のアプリケーションレベルの仕様を探すとき
- CLI、agent call、ログ、feedback、session／run、通知、またはサブコマンドの正本仕様へ進む入口を確認するとき
- 複数の仕様書にまたがる責務分担や、共通規約と個別仕様の適用範囲を判断するとき

## Do not read this when
- 特定仕様書の本文内容や実装上の詳細だけを確認したいときは、該当する個別仕様書を直接読む
- 実装コード、テスト、設定データ構造、または外部契約の正確な定義だけを調べるときは、それぞれの対象ファイルや委譲先の正本を直接読む
- INDEX.md の生成規則や目次更新処理だけを確認したいときは、インデクシング仕様を直接読む

## hash
- a34605bfeb53170f32c5faaa86f63937df73c5a8e8cd1c729363b71429265542
