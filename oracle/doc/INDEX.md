...
## Summary
- cmoc アプリケーションの正本仕様をまとめる入口。CLI 共通規則、状態管理、ログ・フィードバック、oracle/realization の責務、Codex 呼び出し、入力・通知などの横断仕様と、各サブコマンド固有仕様を扱う。
- `sub_command` 配下は、session、run、oracle、realization、tui、doctor など個別サブコマンドの目的・前提条件・実行手順・結果契約を確認するための下位入口である。

## Read this when
- cmoc のアプリケーション仕様を横断的に調査するとき
- 複数のサブコマンドにまたがる共通契約や責務分担を確認するとき
- 個別サブコマンドの仕様を探し、該当する下位文書へ進むとき

## Do not read this when
- 対象のサブコマンドや仕様文書が既に特定されており、その文書を直接読めば足りるとき
- 実装コードやテストコードの具体的な挙動を確認したいとき
- INDEX 生成の対象本文ではなく、生成済みの案内情報だけを確認したいとき

## hash
- 53e8774fc7d5370aa898f0522fd7317f82efc3329ffdea72ed222a21f0c32c8c
