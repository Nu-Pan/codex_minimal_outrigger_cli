...
## Summary
- cmoc の利用 workflow、横断的なアプリケーション規則、機能別仕様をまとめ、個別サブコマンドの仕様へ進む入口となる。
- 共通規則は Codex 呼び出し、session/run、ログと報告、入力受け渡し、feedback、インデクシング、通知などを扱い、個別仕様は各コマンドの引数・事前条件・手順・結果を定める。
- 複数の動作にまたがる仕様の分担や、変更を反映すべき規則を調べるために使う。

## Read this when
- cmoc の動作を変更・調査し、適用する共通規則や個別コマンド仕様の所在を判断するとき。
- Codex 呼び出し、状態管理、表示と記録、入力受け渡し、feedback、インデクシング、通知など、複数コマンドに関わる契約を確認するとき。
- 利用 workflow から workload を選ぶときや、session/run の共通 lifecycle を確認するとき。

## Do not read this when
- 特定コマンドの引数・実行手順・固有レポートだけが論点なら、そのコマンドの仕様へ直接進み、必要な共通規則だけを参照するとき。
- 委譲済みの正確な schema や prompt 構築内容だけを調べる場合は、その内容を定める実装仕様を直接読むとき。
