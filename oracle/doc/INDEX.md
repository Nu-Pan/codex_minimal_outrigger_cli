...
## Summary
- cmoc のアプリケーション仕様を収録するディレクトリ。CLI の実行境界、Codex 呼び出し、ログ・エラー処理、feedback、session/run、サブコマンド、通知など、個別機能の正本仕様を確認する入口であり、各機能の詳細仕様へ進むためのルーティング先となる。

## Read this when
- cmoc のアプリケーションレベルの挙動仕様、共通契約、サブコマンド仕様の入口を探すとき
- CLI 実行、Codex 呼び出し、出力・エラー処理、feedback、session/run、通知、INDEX.md 生成など、複数のアプリケーション仕様にまたがる要件を確認するとき
- 個別仕様の責務分担や、関連する正本文書への進み先を確認するとき

## Do not read this when
- 特定機能の詳細な挙動だけを確認する場合は、このディレクトリ全体ではなく対応する個別仕様を直接読む
- 実装配置、テスト規約、テスト実行手順、開発環境だけを確認する場合は、対応する設計・テスト・開発環境の資料を直接読む
- 保存済み report や実行時生成物の具体的内容だけを調査する場合は、該当する生成物を直接調べる

## hash
- e4a29b32216946214e1b16624e4c5b7e5c5ae28a3d397bef9cc75c534b3a2282
