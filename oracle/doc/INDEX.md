...
## Summary
- cmoc 全体のアプリケーション仕様を束ねる入口。個別サブコマンドや共通規約を横断して、どの正本断片を読むべきかを切り分けるときに使う。
- 各種仕様断片の相互関係を見たいときにまず参照する層で、通常の CLI 振る舞い、状態、ログ、補完、外部モデル連携のうち、どの文書へ進むかを判断するための案内を担う。

## Read this when
- cmoc の全体仕様のどこを読むべきか迷っているとき。
- 複数の仕様断片にまたがる振る舞いの境界、優先関係、責務分担を確認したいとき。
- 個別サブコマンドや共通ルールを扱う前に、上位の入口から対象文書を絞り込みたいとき。

## Do not read this when
- すでに読むべき個別の正本断片が分かっているとき。
- 特定サブコマンドの詳細仕様、実装手順、状態遷移だけを確認したいとき。
- INDEX.md そのものの生成や更新ルールだけを見たいとき。

## hash
- e60d9535621a1818fb2ad407fc1bc72fde2d4447582082e69808df98ca75abd7
