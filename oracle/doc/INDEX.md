...
## Summary
- cmoc のアプリケーション仕様を分野別に定義する正本仕様群。CLI 実行、サブコマンド、session・run、feedback、editor input、ログ・エラー、通知などの挙動仕様への入口となる。

## Read this when
- cmoc の個別アプリケーション機能の挙動仕様を探すとき。
- CLI 自動補完、サブコマンド実行、feedback、session・run lifecycle、editor input、ログ・エラー、割り込み、Windows toast 通知などの仕様を実装・変更・レビューするとき。
- 対象機能に対応する正本仕様の担当範囲と、詳細を読むべき入口を判断するとき。

## Do not read this when
- INDEX.md の生成・更新規則そのものを確認するときは、indexing の仕様を読む。
- 単一仕様書の詳細な挙動だけを確認したいときは、ルーティング後に対応する仕様書を直接読む。
- 実装コードやテストの具体的な挙動を調べるときは、対応する realization file を直接読む。
- 開発環境、設計規則、oracle・realization の分類など、アプリケーション挙動以外の共通規則だけを確認するとき。

## hash
- 7982e5ab0e78431ee666216455400fe43355b376326cb5d464ecde92942f5e97
