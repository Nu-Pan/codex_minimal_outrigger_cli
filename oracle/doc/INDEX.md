...
## Summary
- cmoc のアプリケーション仕様を構成する正本文書群への入口。CLI 自動補完、Codex 呼び出し、ログ、feedback、session/run、各種サブコマンドなど、個別機能の挙動・責務・状態遷移を定義する仕様を扱う。
- 共通仕様と個別仕様を分担しており、対象機能の実装・変更・レビュー時に該当する正本仕様へ進むための起点となる。

## Read this when
- cmoc のアプリケーション挙動に関する正本仕様を調査・実装・変更・レビューするとき。
- 複数のサブコマンド、session/run、feedback、ログ、agent call などにまたがる仕様の入口や担当文書を確認するとき。

## Do not read this when
- 特定の仕様本文の詳細だけを確認する場合は、対象機能に対応する個別仕様を直接読む。
- INDEX.md の生成・更新規則やルーティング処理自体を確認する場合は、インデクシング仕様を直接読む。
- 実装ファイルやテストの具体的な挙動だけを調べる場合は、対応する realization の実装・テストを直接読む。

## hash
- 1270da61723529057c9ea8808719f2b56f0b2a37198781533f28f7838b2c2add
