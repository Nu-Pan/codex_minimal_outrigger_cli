...
## Summary
- cmoc のアプリケーション仕様を集約するディレクトリで、CLI 実行契約、出力・ログ、doctor 前処理、自動補完、feedback、session/run、oracle・realization、サブコマンドなどの正本仕様への入口を提供する。
- 各仕様書は、共通契約や個別サブコマンドの挙動、状態管理、エラー・中断処理、通知、インデクシングといった責務ごとに分担されており、実装・変更・レビュー時に対象の仕様へルーティングするために使う。

## Read this when
- cmoc のアプリケーション仕様全体から、対象機能に対応する正本仕様や責務分担の入口を探すとき
- CLI サブコマンド、Codex 呼び出し、出力・ログ、feedback、session/run、oracle・realization、通知など複数の仕様領域にまたがる挙動を確認するとき

## Do not read this when
- 特定の仕様書が明確な場合は、このディレクトリ全体ではなく該当する仕様書を直接読むとき
- 実装詳細、テスト手順、保存済み成果物、外部契約など、本文が直接の正本ではない事項を確認するときは対応する実装・テスト・委譲先の資料へ進むとき

## hash
- cd9aab5cfff4cbcc4ae8cf24e1a5d42568c32fb89f8573e9737069cd0b39e914
