...
- `CmocConfigCodex.model_providers` は、model provider ID を `CodexModelProviderConfig` へ対応付ける
- `CodexModelProviderConfig.settings` は、provider-local key を null 以外の `JsonTomlValue` へ対応付ける
- null でない model provider ID は `CmocConfigCodex.model_providers` に存在しなければならない。存在しない場合は Codex CLI を起動する前にエラーとする
- Codex CLI の組み込み model provider を明示的に選択する場合を含め、provider-local 設定が不要なら `settings` が空の `CodexModelProviderConfig` を provider 定義として使用してよい
...
- `JsonTomlValue` は null を除く JSON value の型であり、かつ TOML value へ一意に符号化できなければならない
