...
報告基準の意味は本書を正本とする。正確な agent 向け文面は、`oracle/src/oracle/prompt_builder/policy/feedback_reporting.py:7` の `build_feedback_reporting_policy` へ委譲する。完全 prompt への配置は、`oracle/src/oracle/prompt_builder/complete_prompt.py:41` の `build_complete_prompt` へ委譲する。
...
input は、`oracle/src/oracle/feedback/reporter_input.json:1` の root schema（JSON Pointer `#`）に適合する JSON object とする。同 schema を tool discovery と受け入れ検査の両方に使用する。repository、call ID、保存先、または capability を agent input に追加してはならない。
