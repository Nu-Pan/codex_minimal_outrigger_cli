...
- editor work file の初期値は、人間が直接記入する場合に向けた、長くても数行程度の簡単な説明を HTML コメント内に記したものとする。正確な文面は、`{{cmoc-root}}/oracle/src/oracle/prompt_builder/editor_input.py` の `build_prompt_editor_input_initial_text` へ委譲する。
...
1. cmoc は、`build_prompt_editor_input_initial_text` の生成結果を初期値とする editor work file を作成する。
