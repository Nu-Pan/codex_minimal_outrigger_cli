...
## Summary
- コンソール表示、サブコマンドの JSON Lines ログ、Codex CLI 呼び出しログに関する正本仕様。時間・パスの表記、ログの保存場所とイベント要件、コンソール通知の Markdown 形式、開始・ステップ・呼び出し・完了時の必須情報を定義する。

## Read this when
- コンソール出力の時間・パス表記を実装または確認するとき
- サブコマンドログの保存場所、形式、イベント、flush 要件を扱うとき
- サブコマンドの開始通知、ステップ通知、Codex CLI 通知、完了サマリーを実装または確認するとき

## Do not read this when
- コンソールやファイルログの仕様を扱わず、別のアプリケーション仕様や内部処理だけを変更・確認するとき
- 具体的な実装箇所やテスト方法を調べる場合は、この正本仕様ではなく対応する realization 実装・テストを直接確認するとき

## hash
- e5b09f884e06cbe4b0ea32e3f08488cfc75de2af702c07353780ff62b5d3a929
