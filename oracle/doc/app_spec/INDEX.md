...
## Summary
- cmoc が agent に渡すプロンプトの標準を定める。プロンプト生成の責務、許容する記法、自然言語の既定言語を確認したいときに読む。
- 特に、`build_*_parameter` による動的構築を前提にした扱い、`{{...}}` プレースホルダ、`<cmoc_block>` / `<cmoc_ref>` の参照記法、Codex CLI で使う文章の日本語運用を押さえるための入口である。

## Read this when
- agent call に渡すプロンプトの生成規範や、realization 側での加工可否を確認したいとき。
- Markdown と GFM を前提にした cmoc 固有のプロンプト記法や、ブロック参照の表現を統一したいとき。
- Codex CLI で扱う日本語・英語の境界を確認したいとき。

## Do not read this when
- 個別機能ごとのプロンプト本文やテンプレートの内容を確認したいときは、該当する生成元の oracle src 側を直接読む。
- 実装側の具体的な組み立て処理やファイル分割の判断を見たいだけなら、この文書ではなく関連する実装や別の仕様を読む。
- 既に定まった出力形式の機械的な項目名や型だけを確認したいときは、ここではなく該当する schema 定義を読む。

## hash
- 5c80585b538dad342d878907c93f0dc7fcf52ed777e022e9dbf8cfb328e34d93
