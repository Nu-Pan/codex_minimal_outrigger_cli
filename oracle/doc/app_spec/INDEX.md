...
## Summary
- cmoc が agent 呼び出し時に渡すプロンプトの作り方と、その文面に対する扱いを定める。動的に組み立てる正本の責務、realization 側で加工してよい例外、自然言語部分の既定言語を確認したいときに読む。
- プロンプト生成の実装方針や、実際に渡す文面を変更するか判断したいときの入口になる。言語規範まで含めて、どの部分を oracle src に寄せ、どの部分を realization 側で補正できるかを見分けるための案内にする。

## Read this when
- agent call に渡すプロンプトの生成方針を決めたいとき
- realization 側でプロンプトを加工してよいか判断したいとき
- Codex CLI が扱う自然言語部分の既定言語を確認したいとき

## Do not read this when
- すでに使う build_*_parameter 関数や各種プロンプト組み立て処理が特定できていて、実装詳細だけを追いたいとき
- プロンプトの文面ではなく、別の入出力仕様やコマンド動作を確認したいとき
- 言語規範ではなく、個別機能の手順や制御フローだけを見たいとき

## hash
- 70a58af4c800fbafc5d85285af6f1af596417bc51c4b223509d876e9011293f7
