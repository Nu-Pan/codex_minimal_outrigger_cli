...
## Summary
- cmoc が agent に渡すプロンプトの作り方と表記規則を定める、cmoc 固有の正本仕様断片。`oracle/src/oracle/acp_builder` の動的構築結果を基本にしつつ、プレースホルダ表記、`cmoc_block` / `cmoc_ref` による参照表現、自然言語部分の既定言語を揃える。

## Read this when
- agent call に渡すプロンプトの生成方法や加工可否を確認したいとき。
- プロンプト内の参照構造の書き方や、プレースホルダの記法を合わせたいとき。
- Codex CLI が扱う日本語・英語の使い分けを判断したいとき。

## Do not read this when
- 一般的な CLI 振る舞いや実装詳細を知りたいだけで、プロンプト規範そのものは関係しないとき。
- この規範を実装する側の関数分割や内部 helper の設計を知りたいとき。
- 出力 schema やコマンド引数の細部だけを確認したいとき。

## hash
- 6aa34fe51169c3fcfe33e571ca66ceed2baf4424201d0eb28fa7128b65a6bf31
