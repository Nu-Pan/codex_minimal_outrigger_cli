...
## Summary
- ユーザー入力プロンプトと自動生成プロンプトを組み合わせて AI Agent CLI/TUI を起動する `cmoc tui` サブコマンドの正本仕様断片。エディタ入力、agent call によるパラメータ解決、完全プロンプト生成、Codex CLI 起動時に持ち込む実行規則の境界を扱う。

## Read this when
- 対話的に任意プロンプトを入力して AI Agent CLI/TUI を起動するサブコマンドの挙動を確認・実装・テストする場合。
- ユーザーのオリジナルプロンプトをどのエディタで、どの保存先に、どの初期本文で入力させ、読み出し時にコメントや空白をどう扱うかを確認する場合。
- `cmoc tui` が agent call で決定するパラメータや、オリジナルプロンプトを完全プロンプトの補助指示へ注入する流れを確認する場合。
- Codex CLI を TUI として起動する際に、既存の Codex 実行規則からどの要素を引き継ぐかを確認する場合。

## Do not read this when
- 非対話の `codex exec` 起動規則そのもの、preflight validation、profile、ファイルアクセス制限、モデル指定、プロンプト渡しの詳細だけを確認したい場合は、Codex 実行規則の仕様を直接読む。
- agent call のパラメータ仕様や prompt builder の詳細な入出力だけを確認したい場合は、それぞれの正本となる実装仕様を直接読む。
- サブコマンド一覧、CLI 全体の引数体系、または `tui` 以外のサブコマンドの挙動を調べる場合は、より上位または該当サブコマンドの仕様へ進む。
- oracle file と realization file の責務、INDEX エントリー作成規則、または仕様記述の一般原則を確認したい場合は、このサブコマンド仕様ではなく基礎規約を読む。

## hash
- f1c04c02224b5946fc104aee0864654f1ff03d85bc6d2c922396f9f90de98ef1
