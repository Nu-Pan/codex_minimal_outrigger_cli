...
## Summary
- `<repo-root>` を cmoc で作業可能にする初期化サブコマンドの仕様断片。引数なしで実行され、`.cmoc` 配下を git 追跡対象外にする保証と、その時点までの差分を commit する流れを扱う。
- `.cmoc` を ignore 対象にするための `.gitignore` 追加、既存 tracked ファイルの追跡解除、`git ls-files` と `git check-ignore` による完了判定を確認する入口になる。

## Read this when
- `cmoc init` の実行内容、引数、事前条件を確認したいとき。
- 初期化時に `<repo-root>/.cmoc` を git 管理から外す仕様、または将来作成される `.cmoc` 配下ファイルを ignore 対象にする判定条件を確認したいとき。
- `.cmoc` 配下の既存 tracked ファイルをどう扱うべきか、または `git rm --cached` 相当の処理が必要かを判断したいとき。
- `cmoc init` 実行中に発生した差分を commit する流れを扱う実装やテストを変更するとき。

## Do not read this when
- `cmoc init` 以外のサブコマンドの仕様を確認したいとき。
- cmoc 全体のパス用語、`<repo-root>` や `<work-root>` の定義そのものを確認したいとき。
- `.cmoc` 配下に保存される具体的な状態ファイルやデータ構造の仕様を確認したいとき。
- 一般的な git ignore や git commit の実装方針だけを確認したいとき。

## hash
- 511fa8def29176fc65bc7c7c807bfa436d6a881d78201f9f79d75f53277f1569
