...
- 以下のファイルは「コンフリクト自動解決対象」とする
    - `INDEX.md`
- `INDEX.md` がコンフリクトした場合
    - 衝突箇所は単純に削除して解決すること
    - `INDEX.md` は cmoc による自動生成が可能なので削除でも良い
- 前述した「想定外の差分」が無ければ、マージコンフリクトが発生しうるのは「コンフリクト自動解決対象」だけのはずである
    - 通常モードでは想定外の差分が有る場合、処理を中断するのでマージまでたどり着かない
    - 強制モードでは revert によって無理やり想定内の差分に収める
- 「コンフリクト自動解決対象」以外のコンフリクトが発生した場合
    - cmoc が解決を試みることはせず、最後のレポートの一部としてユーザーに報告する
