...
3. apply ループ
    1. 調査対象となるファイルを列挙する
    2. Codex CLI に、列挙したファイルリストを元に所見をリストアップさせる
    3. 所見リスト改善ループ
        1. Codex CLI に、所見リストを改善させる
        2. 改善点がなければここで所見リスト改善ループを抜ける
        3. 所見リスト改善ループ先頭に戻る
    4. 改善後の所見リストが空であれば、所見なしとしてループを終了する
    5. 修正作業ループ（改善後の所見リストに対する for-each）
        1. 所見 1 つに対する修正作業を Codex CLI に依頼する
...
- `cmoc apply fork` は、所見が残っていないことを保証しない
- `cmoc apply fork` は、全ての所見を漏れなく発見することは保証しない（あくまでベストエフォート的に振る舞う）
...
- `CmocConfigApplyFork` で、各ループの最大反復回数を受け取る
...
- 規則は以下の通り
    - 改善済みの最終的な所見リスト上、修正内容と関係すると判定されたファイルはダーティフラグを true に、そうではないファイルは false にする
    - Codex CLI による修正作業の結果として差分が発生したファイルはダーティーフラグを true にする

## 所見リストアップの仕様

- 具体的な `codex exec` 呼び出しの仕様は `build_apply_fork_file_finding_enumeration_parameter` を正本とする
...
- 改善後の所見リストに含まれる所見 1 件ごとに、独立した agent call を行う
