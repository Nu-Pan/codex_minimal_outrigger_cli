...
- realization code を作成または変更し、対応する oracle file が存在する場合は、コメントに `{{work-root}}` 起点の oracle file path を書く。
- 参照コメントは意味仕様の所在を示すために使用し、oracle file の本文または prompt 文面を realization code へ複製するために使用してはならない。
