...
- 型チェッカーと実行時挙動を揃えるため、`from __future__ import annotations` は使わない
...
- 文字コードに起因するトラブルシューティングの支障を避けるため、ログメッセージは英語で書く
...
- 非公開の識別子とは、定義した範囲の外へ公開しない識別子を指す。例えば、モジュール外やクラス外に公開しないものが該当する
