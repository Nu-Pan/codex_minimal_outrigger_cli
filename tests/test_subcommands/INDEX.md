...
- テストケース、共通ヘルパー、公開 API は持たず、パッケージの入口だけを提供します。

## Read this when

- `tests.test_subcommands` を Python パッケージとして import できる理由を確認したいとき。
- 個別テストやヘルパーに進む前に、この入口だけ把握したいとき。
- このファイルが空のパッケージ初期化用モジュールであることを確認したいとき。
