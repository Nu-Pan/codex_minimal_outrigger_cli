...
- `<cmoc-root>/tests/test_subcommands/__init__.py` は `tests/test_subcommands` を Python パッケージとして扱うための初期化ファイルです。
- 内容は実質的に空で、相対インポートやテストパッケージ認識のために置かれています。

## Read this when

- `<cmoc-root>/tests/test_subcommands` を Python パッケージとして成立させる意図を確認したいとき。
- このテスト群で相対インポートが使えるかを確認したいとき。
- テスト用ディレクトリの初期化ファイルが必要かどうかを判断したいとき。

## Do not read this when

- このファイルに実装ロジックやテスト本体が書かれていることを期待するとき。
- `<cmoc-root>/tests/test_subcommands` 配下の具体的な検証内容やヘルパー関数を探したいとき。
- CLI 実装や本体の振る舞いを確認したいとき。
