...
- このファイル自体にはテストロジックはなく、サブコマンド関連テスト群のパッケージ境界だけを示します。
...
- 個別テストケースの本文やアサーションだけを確認したいとき。
- `tests/test_subcommands` 配下の `helpers.py` や他の `test_*.py` を直接読みたいとき。
- サブコマンド本体の実装や CLI の仕様を確認したいとき。
