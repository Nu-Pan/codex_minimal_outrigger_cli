...
- `<cmoc-root>/tests/test_subcommands/__init__.py` は `tests.test_subcommands` を Python パッケージとして成立させるための空ファイルです。
- このファイル自体にはテストケースや実装ロジックはなく、同ディレクトリ配下の `test_*.py` や `helpers.py` を import 可能にする役割だけを持ちます。
